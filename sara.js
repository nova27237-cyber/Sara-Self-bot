...
// ============================================
// 📤 اکسپورت برای Worker - نسخه ساده و کامل
// ============================================
...
<head>
  ...
  <style>
    body { font-family: Arial; text-align: center; padding: 50px; background: #f5f5f5; direction: rtl; }
    .container { background: white; padding: 40px; border-radius: 20px; max-width: 400px; margin: auto; }
    input { padding: 12px; width: 90%; border-radius: 10px; border: 2px solid #e84393; margin: 10px 0; font-size: 16px; }
    button { padding: 14px 40px; background: #e84393; color: white; border: none; border-radius: 10px; font-size: 18px; cursor: pointer; }
    button:hover { background: #c72a7a; }
...
<body>
  <div class="container">
    ...
    <br>
    <button onclick="login()">📱 ارسال شماره</button>
    <div id="status"></div>
  </div>
  <script>
    async function login() {
      ...
      if (!phone || phone.length < 10) {
        ...
        status.innerHTML = '❌ شماره معتبر وارد کنید';
        return;
      }
      try {
        ...
        if (data.success) {
          ...
          status.innerHTML = '✅ شماره ذخیره شد! کد تایید به تلگرامت رسید.';
    ...
    if (url.pathname === '/setphone' && request.method === 'POST') {
      try {
        ...
        if (!phone || !phone.match(/^\\+?[0-9]{10,15}$/)) {
          return new Response(JSON.stringify({ error: 'شماره نامعتبر' }), { status: 400 });
        }
    ...
    if (url.pathname === '/setcode') {
      ...
      if (!code || !code.match(/^[0-9]{5}$/)) {
        return new Response('❌ کد 5 رقمی وارد کن', { status: 400 });
      }
