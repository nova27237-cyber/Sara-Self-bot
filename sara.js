...
class MemoryVault {
  ...
  calculateImportance(text) {
    const importantWords = ['عشق', 'دوست', 'ازدواج', 'زندگی', 'بغض', 'اشک', 'دل', 'قلب', 'مرگ', 'زندگی', 'مرگ', 'مهم', 'حتماً', 'قول'];
    let score = 0;
  ...
  updatePreferences(userId, text) {
    const prefs = this.userPreferences.get(userId) || { topics: [], style: 'normal', favoriteWords: [] };
    const topics = ['عشق', 'زندگی', 'کار', 'خانواده', 'دوستان', 'سلامتی', 'سرگرمی', 'فیلم', 'موسیقی', 'سفر', 'مدرسه', 'دانشگاه'];
  ...
  getRelevantMemories(userId, query, limit = 15) {
    const memories = this.memories.get(userId) || [];
    const scored = memories.map(m => ({
      ...m,
      score: this.similarity(m.text, query) + (m.importance || 0)
    }));
  ...
  getUserPreferences(userId) {
    return this.userPreferences.get(userId) || { topics: [], style: 'normal', favoriteWords: [] };
  }
...
🌸 **با عشق سارا** 💕
  ...
  updateRelationship(userId, userMood) {
    ...
    // افزایش صمیمیت بر اساس خلق کاربر
    if (userMood === 'flirty' || userMood === 'romantic') {
      relationship.intimacy = Math.min(1, relationship.intimacy + 0.08);
      relationship.trust = Math.min(1, relationship.trust + 0.05);
    } else if (userMood === 'happy') {
...
class LearningEngine {
  ...
  suggestResponse(text, userId) {
    ...
    // الگوی کلی (همه کاربران) - با امتیاز بالا
    let bestMatch = null;
    let bestScore = 0;
    for (const [key, data] of this.patterns) {
...
// ============================================
// 📦 ۵. کلاس مدیریت سشن (SessionManager)
// ============================================
