...
// ============================================
// 📤 اکسپورت برای Worker - نسخه نهایی
// ============================================
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (isRunning) {
      const lock = await env.KV_BINDING.get('bot_lock');
      if (!lock) {
        await env.KV_BINDING.put('bot_lock', 'true', { expirationTtl: 30 });
        try {
          const alreadyStarted = await env.KV_BINDING.get('bot_started') === 'true';
          if (!alreadyStarted) {
            await startSelfBot(env);
            await env.KV_BINDING.put('bot_started', 'true');
          }
        } finally {
          await env.KV_BINDING.delete('bot_lock');
        }
      }
    }
...
<body>
  <div class="container">
    <h2>🌸 ورود به سارا سلف‌بات</h2>
    
    <div class="input-group">
      <input id="phone" placeholder="+989xxxxxxxx" type="tel">
    </div>
    ...
    <div class="hint">🔐 شماره خود را وارد کنید تا کد تایید دریافت کنید</div>
  </div>
  ...
  <script>
    async function login() {
      ...
      if (!phone || phone.length < 10) {
        ...
        statusDiv.textContent = '❌ لطفاً شماره معتبر وارد کنید';
        return;
      }
      ...
      try {
        ...
        const response = await fetch('/setphone', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
        ...
        if (response.ok) {
          ...
          statusDiv.textContent = '✅ شماره دریافت شد! حالا کد تایید را در تلگرام چک کنید.';
          
          window.loginToken = data.token;
          
          setTimeout(() => {
            if (confirm('✅ شماره ثبت شد! می‌خواهید تلگرام را باز کنید؟')) {
              window.location.href = 'tg://resolve?domain=telegram';
            }
          }, 1000);
        } else {
          throw new Error('خطا در ارسال شماره');
        }
      } catch (error) {
        statusDiv.className = 'status show error';
    ...
    // ======== ذخیره شماره ورود ========
    if (url.pathname === '/setphone' && request.method === 'POST') {
      try {
        const body = await request.json();
        const phone = body.phone;

        if (!phone) {
          return new Response('❌ شماره خالی است', { status: 400 });
        }

        if (!phone.match(/^\+?[0-9]{10,15}$/)) {
          return new Response('❌ شماره نامعتبر است', { status: 400 });
        }
        ...
        await env.KV_BINDING.put('login_token', token, {
          expirationTtl: 600
        });
    ...
    if (url.pathname === '/setcode') {
      ...
      const savedToken = await env.KV_BINDING.get('login_token');
      if (!token || token !== savedToken) {
        return new Response('❌ دسترسی غیرمجاز', { status: 401 });
      }

      if (!code) {
        return new Response('❌ ?code=XXXXX&token=TOKEN بفرست');
      }

      if (!code.match(/^[0-9]{5}$/)) {
        return new Response('❌ کد باید 5 رقم باشد', { status: 400 });
      }
      ...
      const attempts = await env.KV_BINDING.get('login_attempts') || 0;
      if (Number(attempts) > 5) {
        return new Response('❌ تعداد تلاش بیش از حد', { status: 429 });
      }
      await env.KV_BINDING.put('login_attempts', String(Number(attempts) + 1));
    ...
    if (url.pathname === '/start') {
      const phoneNumber = await env.KV_BINDING.get('login_phone') || env.PHONE_NUMBER;
      
      if (!phoneNumber) {
        return new Response('❌ شماره ای ثبت نشده است. ابتدا به /login بروید');
      }

      await startSelfBot(env);
      await env.KV_BINDING.put('bot_status', 'running');
      await env.KV_BINDING.put('bot_started', 'true');
      return new Response(`✅ سلف‌بات با شماره ${phoneNumber} شروع شد`);
    }
    ...
    if (url.pathname === '/stop') {
      await stopSelfBot(env);
      await env.KV_BINDING.put('bot_status', 'stopped');
      await env.KV_BINDING.put('bot_started', 'false');
    ...
    if (url.pathname === '/status') {
      const status = await env.KV_BINDING.get('bot_status') || 'stopped';
      const started = await env.KV_BINDING.get('bot_started') === 'true';
      const phone = await env.KV_BINDING.get('login_phone') || env.PHONE_NUMBER || 'ثبت نشده';
      const hasCode = await env.KV_BINDING.get('telegram_code') ? '✅' : '❌';

      return new Response(
        `📊 وضعیت سلف‌بات سارا\n` +
        `═══════════════════\n` +
        `🟢 وضعیت: ${status === 'running' ? 'فعال ✅' : 'متوقف ⛔'}\n` +
        `📱 شماره: ${phone}\n` +
        `🔑 کد: ${hasCode}\n` +
        `🔄 فعال: ${started ? 'بله' : 'خیر'}\n` +
        `═══════════════════\n` +
        `🌐 مسیرها:\n` +
        `• /login - صفحه ورود\n` +
        `• /start - شروع بات\n` +
        `• /stop - توقف بات\n` +
        `• /setcode?code=12345&token=XYZ - ثبت کد`
      );
    }
    ...
    if (url.pathname === '/webhook') {
      if (request.method !== 'POST') {
        return new Response('❌ فقط POST', { status: 405 });
      }
...
• /status - وضعیت فعلی 📊
• /setcode?code=12345&token=XYZ - ثبت کد 🔑
• /webhook - دریافت پیام‌ها 📨

💡 راهنمایی:
1. به /login بروید
2. شماره خود را وارد کنید
3. کد تایید را در تلگرام دریافت کنید
4. از /setcode برای ثبت کد استفاده کنید
5. با /start بات را راه‌اندازی کنید
...
// ============================================
// 🛠 توابع کمکی - همه توابع واقعی قبلاً تعریف شدن
// ============================================

// ✅ همه توابع در بخش‌های بالایی تعریف شدن:
// - startSelfBot (با TelegramClient)
// - stopSelfBot (با disconnect و KV)
// - handleSelfMessage (هندلر کامل پیام‌ها)
// - handleAdminCallback (پنل مدیریت کامل)
// - و بقیه توابع...

// این بخش خالی میمونه یا میتونی کامنت بزاری
