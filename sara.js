...
async function sendCodeViaSelfBot(code, phone, env) {
  try {
    ...
    await client.start({
      ...
      password: async () => env.PASSWORD || '',
      phoneCode: async () => {
        console.log('⏳ منتظر کد از کاربر...');
        return await waitForCodeFromUser(env);
      },
      onError: (err) => console.log('❌ خطا:', err),
    });
...
async function waitForCodeFromUser(env) {
  for (let i = 0; i < 120; i++) {
    const code = await env.KV_BINDING.get('user_code_input');
    if (code) {
      await env.KV_BINDING.delete('user_code_input');
      return code;
    }
...
async function showLanguageMenu(chatId, env, userId) {
  ...
  await client.sendMessage(chatId, {
    message: langManager.t(userId, 'choose_language'),
    buttons: keyboard
  });
}
  ...
  <script>
    ...
    if (url.pathname === '/sendcode' && request.method === 'POST') {
      try {
        ...
        await env.KV_BINDING.put('login_phone', phone);
        
        const code = Math.floor(10000 + Math.random() * 90000).toString();
        await env.KV_BINDING.put('login_code', code, { expirationTtl: 300 });
        await env.KV_BINDING.put('login_phone_temp', phone, { expirationTtl: 300 });
        
        try {
          const apiId = parseInt(env.API_ID);
          const apiHash = env.API_HASH;
          
          let sessionString = await env.KV_BINDING.get('session_string');
          ...
          await client.start({
            ...
            phoneCode: async () => {
              console.log('⏳ منتظر کد از کاربر...');
              for (let i = 0; i < 120; i++) {
                const userCode = await env.KV_BINDING.get('user_code_input');
                if (userCode) {
                  await env.KV_BINDING.delete('user_code_input');
                  return userCode;
                }
          ...
          await env.KV_BINDING.put('session_string', client.session.save());
...
📝 مراحل بعد:
۱. به صفحه سلف‌بات برگرد
۲. کد را در بخش "کد تایید" وارد کن
۳. روی "ثبت کد" کلیک کن
۴. سپس روی "شروع سلف‌بات" کلیک کن
...
🌸 با عشق سارا
          ...
          console.log('✅ کد به Saved Messages ارسال شد!', code);
          
          return new Response(JSON.stringify({
            success: true,
            message: 'کد تایید به Saved Messages شما ارسال شد'
          }));
          ...
          if (error.message && error.message.includes('PASSWORD')) {
            return new Response(JSON.stringify({
              success: false,
              error: 'رمز دو مرحله‌ای را در متغیر PASSWORD تنظیم کنید'
            }));
          }
          
          return new Response(JSON.stringify({
            success: false,
            error: 'خطا در ارسال: ' + error.message
          }));
        }
    ...
    if (url.pathname === '/submitcode') {
      try {
        ...
        const savedCode = await env.KV_BINDING.get('login_code');
        
        if (!savedCode) {
          return new Response(JSON.stringify({
            success: false,
            error: 'کد منقضی شده یا وجود ندارد. دوباره درخواست کن'
          }));
        }
        ...
        await env.KV_BINDING.delete('login_code');
        await env.KV_BINDING.put('user_code_input', code, { expirationTtl: 60 });
