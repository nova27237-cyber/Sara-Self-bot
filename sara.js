...
// ===== کلاس مدیریت سشن (یک بار تعریف شده) =====
...
class SessionManager {
  ...
  async save(session) {
    await this.env.KV_BINDING.put(this.sessionKey, session);
  }
  ...
  try {
    ...
    await client.start({
      ...
      phoneCode: async () => {
        ...
        console.log('⏳ منتظر کد ورود از پنل...');
        for (let i = 0; i < 60; i++) {
          const telegramCode = await env.KV_BINDING.get('telegram_code');
          if (telegramCode) {
            console.log('✅ کد دریافت شد:', telegramCode);
            await env.KV_BINDING.delete('telegram_code');
            return telegramCode;
          }
          await new Promise(resolve => setTimeout(resolve, 5000));
        }
        throw new Error('زمان انتظار کد ورود تمام شد');
      },
    ...
    // اگر ماژول multiAccountManager تعریف شده است
    if (typeof multiAccountManager !== 'undefined' && multiAccountManager) {
...
async function stopSelfBot(env) {
  ...
  if (selfBotClient) {
    ...
    if (typeof multiAccountManager !== 'undefined' && multiAccountManager) {
      await multiAccountManager.sendToAllManagers('⛔ سلف‌بات متوقف شد!', env);
    }
...
// ============================================
// 📨 ارسال کد با سلف‌بات (به Saved Messages)
// ============================================

let tempClient = null;

async function sendCodeViaSelfBot(code, phone, env) {
  try {
    const apiId = parseInt(env.API_ID);
    const apiHash = env.API_HASH;
    ...
    // ✅ مهم: دیگر منتظر کد از کاربر نمیشیم، مستقیماً خودمون کد رو به عنوان ورودی میدیم
    await client.start({
      phoneNumber: phone,
      password: async () => env.PASSWORD || '',
      phoneCode: async () => {
        console.log('📱 کد تایید به تلگرام ارسال شد!');
        // در اینجا کدی که ساختیم رو مستقیم برمیگردونیم
        return code; 
      },
      onError: (err) => console.log('❌ خطا:', err),
    });
...
🌸 سارا
    ...
    tempClient = client;
    console.log('✅ کد به Saved Messages ارسال شد!');
    return true;
...
<body>
  <div class="container">
    <h1>🌸 سارا</h1>
    <p class="subtitle">سلف‌بات هوشمند</p>
    
    <div class="step">
      <span class="step-number">1</span>
      <strong>شماره خود را وارد کنید</strong>
    </div>
    
    <div class="input-group">
      <label>📱 شماره تلگرام</label>
      <input id="phone" placeholder="+989123456789" value="+989378038736">
    </div>
    
    <button onclick="requestCode()">📨 دریافت کد تایید</button>
    ...
    <div id="codeSection" style="display:none;">
      ...
      <div class="step">
        ...
        <strong>کد دریافتی از تلگرام را وارد کنید</strong>
      </div>
      ...
      <button onclick="submitCode()" class="btn-success">✅ ثبت کد</button>
    </div>
    ...
    <div class="step">
      ...
      <strong>شروع سلف‌بات</strong>
    </div>
    
    <button onclick="startBot()" class="btn-success">▶️ شروع سلف‌بات</button>
    <button onclick="stopBot()" class="btn-danger">⏹️ توقف سلف‌بات</button>
    <button onclick="checkStatus()" class="btn-warning">📊 وضعیت</button>
    
    <div id="status"></div>
    
    <div class="info-text">
      💡 کد تایید به <strong>Saved Messages</strong> شما ارسال خواهد شد
    </div>
  </div>
  ...
  <script>
    ...
    async function requestCode() {
      ...
      if (!phone || phone.length < 10) {
        showStatus('❌ شماره را درست وارد کن!', 'error');
        return;
      }
      
      showStatus('⏳ در حال ارسال کد به Saved Messages شما...', 'info');
      ...
      try {
        ...
        if (data.success) {
          showStatus('✅ کد تایید به Saved Messages شما ارسال شد!', 'success');
    ...
    async function submitCode() {
      ...
      if (!code || code.length < 5) {
        showStatus('❌ کد ۵ رقمی رو وارد کن', 'error');
        return;
      }
      
      showStatus('⏳ در حال ثبت کد...', 'info');
      
      try {
        const res = await fetch('/submitcode?code=' + encodeURIComponent(code));
        const data = await res.json();
        
        if (data.success) {
          showStatus('✅ کد ثبت شد! حالا روی "شروع سلف‌بات" کلیک کن', 'success');
        } else {
          showStatus('❌ ' + data.error, 'error');
        }
      } catch(e) {
    ...
    async function startBot() {
      showStatus('⏳ در حال شروع سلف‌بات...', 'info');
      try {
    ...
    // ============================================
// 📨 ارسال کد تایید (اصلاح شده)
// ============================================
if (url.pathname === '/sendcode' && request.method === 'POST') {
  try {
    const body = await request.json();
    const phone = body.phone?.trim();
    
    if (!phone) return new Response(JSON.stringify({ success: false, error: 'شماره وارد نشده' }));

    const apiId = parseInt(env.API_ID);
    const apiHash = env.API_HASH;
    
    if (!apiId || !apiHash) {
      return new Response(JSON.stringify({ success: false, error: 'API_ID و API_HASH را در محیط تنظیم کنید' }));
    }

    await env.KV_BINDING.put('login_phone', phone, { expirationTtl: 600 });
    ...
    // تولید کد و ذخیره در KV ✅
    const generatedCode = Math.floor(10000 + Math.random() * 90000).toString();
    await env.KV_BINDING.put(`code_input_${phone}`, generatedCode, { expirationTtl: 300 });

    await client.start({
      phoneNumber: phone,
      password: async () => env.PASSWORD || '',
      phoneCode: async () => {
        console.log('⏳ منتظر دریافت کد از کاربر...');
        for (let i = 0; i < 30; i++) {
      ...
      onError: (err) => console.log('❌ خطای لاگین:', err.message)
    });

    await client.sendMessage('me', {
      message: `🔐 **کد تایید سارا سلف‌بات**\n\n📱 کد شما: \`${generatedCode}\`\n\n⏳ این کد تا ۲ دقیقه اعتبار دارد.\n\n🌸 با عشق سارا`,
      parseMode: 'Markdown'
    });
    ...
    return new Response(JSON.stringify({
      success: true,
      message: 'کد تایید به Saved Messages ارسال شد. لطفاً آن را در مرحله بعد وارد کنید.'
    }));

  } catch (error) {
    console.error('❌ خطای ارسال کد:', error);
    return new Response(JSON.stringify({
      success: false,
      error: error.message.includes('FLOOD') ? 'درخواست زیاد! ۵ دقیقه صبر کن.' : 'خطا در ارسال: ' + error.message
    }));
  }
}

// ============================================
// ✅ ثبت کد (GET) - اصلاح شده
// ============================================
if (url.pathname === '/submitcode') {
  try {
    const code = url.searchParams.get('code');
    const phone = await env.KV_BINDING.get('login_phone'); // شماره رو میخونیم ✅
    
    if (!code) {
      return new Response(JSON.stringify({ success: false, error: 'کد وارد نشده' }));
    }
    if (!phone) {
      return new Response(JSON.stringify({ success: false, error: 'شماره ثبت نشده' }));
    }

    const savedCode = await env.KV_BINDING.get(`code_input_${phone}`); // کلید درست ✅
    
    if (!savedCode) {
      return new Response(JSON.stringify({ success: false, error: 'کد منقضی شده یا وجود ندارد. دوباره درخواست کن' }));
    }
    
    if (code !== savedCode) {
      return new Response(JSON.stringify({ success: false, error: 'کد اشتباه است' }));
    }
    
    await env.KV_BINDING.put('code_verified', 'true', { expirationTtl: 3600 });
    await env.KV_BINDING.delete(`code_input_${phone}`);
    
    return new Response(JSON.stringify({ success: true, message: 'کد تایید شد!' }));
    ...
    // ============================================
// ✅ ثبت کد (GET) - اصلاح شده (فقط این یکی بمونه)
// ============================================
if (url.pathname === '/submitcode') {
  try {
    const code = url.searchParams.get('code');
    ...
    if (url.pathname === '/start') {
      try {
        const verified = await env.KV_BINDING.get('code_verified');
        
        if (!verified) {
          return new Response(JSON.stringify({
            success: false,
            error: 'ابتدا کد تایید را ثبت کنید'
          }));
        }
        ...
        return new Response(JSON.stringify({
          success: true,
          message: `✅ سلف‌بات با شماره ${phone} شروع شد!`
        }));
    ...
    if (url.pathname === '/status') {
      const status = await env.KV_BINDING.get('bot_status') || 'stopped';
      const phone = await env.KV_BINDING.get('login_phone') || 'ثبت نشده';
      const verified = await env.KV_BINDING.get('code_verified') ? true : false;
      const hasSession = !!(await env.KV_BINDING.get('session_string'));
      ...
      return new Response(JSON.stringify({
        ...
        verified,
        hasSession,
        uptime: status === 'running' ? 'در حال اجرا' : 'متوقف'
      }));
    }
    ...
    return new Response(
      ...
      `📱 شماره: ${await env.KV_BINDING.get('login_phone') || 'ثبت نشده'}\n` +
      `\n📋 مسیرها:\n` +
      `• / - صفحه اصلی\n` +
      `• /sendcode - دریافت کد (با POST)\n` +
      `• /submitcode?code=12345 - ثبت کد\n` +
      `• /start - شروع سلف‌بات\n` +
      `• /stop - توقف سلف‌بات\n` +
      `• /status - وضعیت`
    );
