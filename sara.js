...
// ============================================
// 🚀 سارا HYPER FUL ULTIMATE - Worker
// ============================================

console.log('🔥 سارا HYPER FUL ULTIMATE');

// ============================================
// 🤖 تابع startSelfBot (ساده شده برای Worker)
// ============================================

async function startSelfBot(env) {
  console.log('🚀 شروع سلف‌بات...');
...
<head>
  ...
  <style>
    body { font-family: Tahoma; text-align: center; padding: 30px; background: #f0f0f0; }
    .box { background: white; padding: 30px; border-radius: 20px; max-width: 400px; margin: auto; }
    input { width: 90%; padding: 14px; border: 2px solid #e84393; border-radius: 10px; font-size: 16px; }
    button { width: 90%; padding: 14px; background: #e84393; color: white; border: none; border-radius: 10px; font-size: 18px; cursor: pointer; margin-top: 10px; }
...
<body>
  <div class="box">
    <h2>🌸 ورود به سارا</h2>
    <input id="phone" placeholder="+989378038736">
  ...
  <script>
    async function send() {
      ...
      if (!phone || phone.length < 10) {
        ...
        status.textContent = '❌ شماره را درست وارد کن';
        return;
      }
      ...
      try {
        const res = await fetch('/setphone', {
          method: 'POST',
        ...
        if (data.success) {
          status.className = 'ok';
          status.textContent = '✅ شماره ذخیره شد! کد رو از تلگرام بگیر.';
    ...
    if (url.pathname === '/setphone' && request.method === 'POST') {
      try {
        ...
        if (!phone) {
          return new Response(JSON.stringify({ success: false, error: 'شماره خالی است' }));
        }

        const cleanPhone = phone.replace(/\s/g, '');
        
        if (!cleanPhone.match(/^\+?[0-9]{10,15}$/)) {
          return new Response(JSON.stringify({ success: false, error: 'فرمت شماره اشتباه است' }));
        }

        await env.KV_BINDING.put('login_phone', cleanPhone, { expirationTtl: 600 });
        const token = crypto.randomUUID();
        await env.KV_BINDING.put('login_token', token, { expirationTtl: 600 });
    ...
    if (url.pathname === '/setcode') {
      ...
      if (!token || token !== savedToken) {
        return new Response('❌ دسترسی غیرمجاز');
      }

      if (!code || !code.match(/^[0-9]{5}$/)) {
        return new Response('❌ کد ۵ رقمی وارد کن');
      }

      await env.KV_BINDING.put('telegram_code', code, { expirationTtl: 300 });
    ...
    if (url.pathname === '/start') {
      ...
      if (!phone) {
        return new Response('❌ شماره ثبت نشده. اول /login برو');
      }
      ...
      return new Response('✅ سلف‌بات با شماره ' + phone + ' شروع شد!');
    }
    ...
    if (url.pathname === '/status') {
      ...
      return new Response(
        ...
        `• /start - شروع بات\n` +
        `• /stop - توقف بات\n` +
        `• /setcode?code=12345&token=TOKEN - ثبت کد`
      );
    }
