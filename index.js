...
// ⚠️ اینجا باید ENV_VARS داشته باشی! 
// ولی توی کدت این بخش خالی هست!
// یعنی ENV_VARS رو نداری!
...
body{font-family:Arial;text-align:center;padding:50px;background:linear-gradient(135deg,#ff9a9e,#fad0c4)}
.container{background:white;padding:40px;border-radius:20px;max-width:400px;margin:0 auto}
input{width:100%;padding:12px;font-size:18px;border:2px solid #ddd;border-radius:10px;margin:10px 0;text-align:center}
button{width:100%;padding:12px;background:#6c5ce7;color:white;border:none;border-radius:10px;font-size:18px;cursor:pointer}
</style>
</head>
<body>
<div class="container">
...
document.getElementById('loginForm').onsubmit = async function(e) {
  ...
  try {
    const res = await fetch('/submitcode', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ code, phone: '${ENV_VARS.PHONE_NUMBER}' })
    });
    ...
    if (data.success) {
      ...
      result.innerHTML = '❌ ' + (data.error || 'خطا!');
      result.style.color = 'red';
    }
  } catch(e) {
