...
// ✅ توکن واقعی رو بذار:
const BOT_TOKEN = '8568499654:AAFyLZs7Ex9XzIy3wCUb9l9_gYQ2B_rF-Ss'; // ← اینجا توکن رباتته!
...
async function sendMessage(chatId, text, options = {}) {
  try {
    // ✅ از BOT_TOKEN ثابت استفاده کن (نه env)
    const token = '8568499654:AAFyLZs7Ex9XzIy3wCUb9l9_gYQ2B_rF-Ss';
    ...
    if (!response.ok) {
      console.error('❌ خطا در ارسال:', await response.text());
    }
...
async function answerCallbackQuery(callbackId, text, showAlert = false) {
  try {
    const token = env?.BOT_TOKEN || '8568499654:AAFyLZs7Ex9XzIy3wCUb9l9_gYQ2B_rF-Ss';
...
export default {
  async fetch(request, env) {
    ...
    let botStatus = await env.KV_NAMESPACE.get('bot_status');
    if (!botStatus) {
      await env.KV_NAMESPACE.put('bot_status', 'running');  // ← پیش‌فرض: روشن
      botStatus = 'running';
    }
    ...
    if (url.pathname === '/webhook') {
      try {
        ...
        console.log('📨 Webhook دریافت شد:', JSON.stringify(body));
        ...
        if (message && message.text) {
          ...
          if (adminIds.includes(userId)) {
            ...
            if (text === '/status') {
              const stats = await db.getStats();
              const status = botStatus || 'stopped';
              const uptime = await multiAccountManager.getUptime(env);
        ...
        // ✅ حتی اگه خطا باشه، یه پاسخ پیش‌فرض بفرست!
        try {
          const chatId = body.message?.chat?.id;
          if (chatId) {
            await sendMessage(chatId, '🌸 یه مشکلی پیش اومد! دوباره تلاش کن 😊');
          }
        } catch (e) {}
