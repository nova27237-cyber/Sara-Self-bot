...
// ============================================
// سارا سلف‌بات - نسخه نهایی و ساده
// ============================================
...
<head>
  ...
  <style>
    ...
    h1 { color: #6c5ce7; font-size: 2.5em; margin-bottom: 10px; }
    ...
    .online { color: #00b894; font-weight: bold; }
...
<body>
  <div class="container">
    ...
    <div class="status">
      <p>🟢 وضعیت: <span class="online">آنلاین</span></p>
      <p>📱 شماره: +989378038736</p>
    </div>
...
<head>
  ...
  <style>
    ...
    h2 { color: #6c5ce7; margin-bottom: 10px; }
    ...
    .info { margin-top: 20px; color: #b2bec3; font-size: 0.8em; }
...
<body>
  <div class="container">
    ...
    <form id="loginForm">
      <input type="text" id="codeInput" placeholder="کد ۵ رقمی" maxlength="5" inputmode="numeric" pattern="[0-9]*" required>
      <button type="submit" class="btn" id="submitBtn">✅ ارسال کد</button>
    </form>
    <div id="loading" class="loading"><div class="spinner"></div><p style="margin-top:10px;color:#6c5ce7;">در حال ثبت...</p></div>
    <div class="info">📱 شماره: <span>+989378038736</span></div>
  ...
  <script>
    ...
    form.addEventListener('submit', async function(e) {
      ...
      try {
        const response = await fetch('/submitcode', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ code, phone: '+989378038736' })
        });
        const data = await response.json();
        if (data.success) {
    ...
    if (path === '/submitcode' && request.method === 'POST') {
      try {
        ...
        if (!code || code.length < 5) {
          return new Response(JSON.stringify({ success: false, error: 'کد نامعتبر!' }), {
            headers: { 'Content-Type': 'application/json' }
          });
        }
        await env.KV_BINDING.put(`code_input_${phone}`, code);
        return new Response(JSON.stringify({ success: true, message: '✅ کد ثبت شد!' }), {
          headers: { 'Content-Type': 'application/json' }
        });
      } catch (error) {
