// ============================================
// 📁 index.js - سارا HYPER FUL - نسخه کامل Cloudflare Workers
// ============================================
...
class MediaProcessor {
  async analyzeImage(imageBuffer, env) {
    try {
      ...
      const response = await env.AI.run('@cf/meta/llama-3-8b-instruct', {
        image: imageBuffer,
        messages: [
          { role: 'system', content: 'این عکس رو تحلیل کن و توضیح بده' }
        ]
      });
      
      return {
        description: response?.description || 'یه عکس قشنگه 😍',
  ...
  async transcribeAudio(audioBuffer, env) {
    try {
      ...
      return response?.text || 'نتونستم بشنوم 😅';
    } catch (error) {
...
// ============================================
// 🎙️ تبدیل به ویس با سرویس‌های رایگان
// ============================================
  ...
  try {
    ...
    try {
      ...
      const response = await fetch(
        ...
        {
          method: 'GET',
          headers: { 'Accept': 'audio/ogg', 'User-Agent': 'Mozilla/5.0' }
        }
      );
      if (response.ok) {
        const audioData = await response.arrayBuffer();
          ...
          body: `
            <speak version="1.0" xmlns="http://www.w3.org/2001/10/synthesis" xml:lang="fa-IR">
              <voice name="Microsoft Server Speech Text to Speech Voice (fa-IR, Hedieh)">
                <prosody rate="0.95" pitch="1.1">${text}</prosody>
              </voice>
            </speak>
          `
        }
    ...
    // ۷. تولید صدای ساده با Sine Wave
    try {
...
function enhanceAudioQuality(audioData, options = {}) {
  try {
    const { normalize = true, amplify = 1.2 } = options;
...
async function cacheVoice(text, audioData, env) {
  try {
    ...
    await env.KV_NAMESPACE.put(key, base64Data, { expirationTtl: 86400 });
  } catch (error) {
...
📚 الگوهای یادگیری: ${learningEngine.patterns.size}
      ...
      if (msg) await client.sendMessage(chatId, { message: `💬 ${msg}` });
...
async function showAdminPanel(chatId, messageId, env) {
  try {
    ...
    const keyboard = {
      inline_keyboard: [
        [{ text: "👥 کاربران", callback_data: "admin_users" }, { text: "📢 ارسال همگانی", callback_data: "admin_broadcast" }],
        [{ text: "🔑 کلیدها", callback_data: "admin_keys" }, { text: "📋 لاگ‌ها", callback_data: "admin_logs" }],
        [{ text: "🔄 ریستارت", callback_data: "admin_restart" }, { text: "🧹 پاکسازی کش", callback_data: "admin_clear_cache" }],
        [{ text: "❌ بستن", callback_data: "admin_close" }]
      ]
    };
    
    if (messageId) {
      await editMessageText(chatId, messageId, text, { reply_markup: JSON.stringify(keyboard) });
    } else {
...
async function showUsersList(chatId, messageId, env) {
  try {
    ...
    const pageUsers = sorted.slice(0, 10);
    ...
    if (pageUsers.length === 0) {
      ...
      if (sorted.length > 10) text += `➕ ... و ${sorted.length - 10} کاربر دیگر\n`;
    }
    
    const keyboard = { inline_keyboard: [[{ text: "🔙 بازگشت", callback_data: "admin_back" }]] };
    await editMessageText(chatId, messageId, text, { reply_markup: JSON.stringify(keyboard) });
  } catch (error) {
...
async function showBroadcastPanel(chatId, messageId, env) {
  ...
  const keyboard = { inline_keyboard: [[{ text: "❌ لغو", callback_data: "admin_close" }]] };
...
async function showLogsPanel(chatId, messageId, env) {
  try {
    ...
    if (logs.length === 0) {
      ...
      recentLogs.forEach(log => { text += `• ${log}\n`; });
    }
    const keyboard = {
      inline_keyboard: [
...
async function answerCallbackQuery(callbackId, text, showAlert = false) {
  try {
    const token = process.env.BOT_TOKEN || globalThis.BOT_TOKEN;
...
async function handleAdminCommand(message, env) {
  ...
  if (!adminIds.includes(userId)) {
    await sendMessage(chatId, '⛔ شما اجازه کنترل ندارید!', { reply_to_message_id: messageId });
    return;
  }
...
let multiAccountManager = new MultiAccountManager();
...
async function handleSelfMessage(event, client, env) {
  try {
    ...
    try {
      ...
      if (audioBuffer) {
        const enhancedAudio = enhanceAudioQuality(audioBuffer, { normalize: true, amplify: 1.2 });
        await client.sendFile(chatId, {
          file: enhancedAudio,
          caption: '🎙️ صدای سارا...',
          forceDocument: false,
      ...
      console.error('❌ خطا در تولید صدا:', voiceError);
    }
  } catch (error) {
    console.error('❌ خطا:', error);
  }
  ...
  for (const adminId of adminIds) {
    try {
      await fetch(`https://api.telegram.org/bot${token}/sendMessage`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ chat_id: adminId, text: `${emojis[type] || '📌'} ${text}` })
      });
    } catch (error) {
...
5️⃣ ✅ ساخت عکس با AI
6️⃣ ✅ ساخت فیلم با AI
7️⃣ ✅ یادگیری از مکالمات
8️⃣ ✅ دیتابیس پیشرفته
9️⃣ ✅ پشتیبانی از گروه
🔟 ✅ مدیریت چند کاربر همزمان
1️⃣1️⃣ ✅ شخصیت سارا
1️⃣2️⃣ ✅ تبدیل متن به صدا با ۸ سرویس
...
async function cleanupCache(env) {
  try {
    const keys = await env.KV_NAMESPACE.list({ prefix: 'voice_' });
    const now = Date.now();
    let deleted = 0;
    for (const key of keys.keys) {
      const metadata = await env.KV_NAMESPACE.getMetadata(key.name);
      if (metadata && (now - metadata.created) > 86400000) {
        await env.KV_NAMESPACE.delete(key.name);
        deleted++;
      }
    }
    console.log(`🧹 پاکسازی کش: ${deleted} فایل حذف شد`);
  } catch (error) {
...
body{font-family:Tahoma,sans-serif;background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);min-height:100vh;padding:20px;color:#fff}
.container{max-width:1200px;margin:0 auto}
.header{text-align:center;padding:30px;background:rgba(255,255,255,0.15);border-radius:20px;backdrop-filter:blur(10px);margin-bottom:30px}
...
.controls{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:10px;margin-bottom:30px}
.btn{padding:15px;border:none;border-radius:10px;background:rgba(255,255,255,0.2);color:#fff;cursor:pointer;font-size:16px;transition:all 0.3s;backdrop-filter:blur(10px)}
...
.logs{background:rgba(0,0,0,0.3);padding:20px;border-radius:15px;max-height:300px;overflow-y:auto;font-family:monospace;font-size:12px}
.log-entry{padding:5px 0;border-bottom:1px solid rgba(255,255,255,0.1)}
.status-badge{display:inline-block;padding:5px 20px;border-radius:20px;font-size:14px}
.status-online{background:#4CAF50}
.status-offline{background:#f44336}
.mood-display{font-size:48px}
@media(max-width:600px){.header h1{font-size:1.8em}.stats-grid{grid-template-columns:1fr 1fr}.controls{grid-template-columns:1fr 1fr}}
</style>
</head>
<body>
<div class="container">
...
<p>نسخه کامل با مدیریت چند اکانت</p>
<br>
<span class="status-badge ${status === 'running' ? 'status-online' : 'status-offline'}">
${status === 'running' ? '🟢 آنلاین' : '🔴 آفلاین'}
</span>
<span style="margin-right:15px;">👥 ${multiAccountManager.managers.length} مدیر</span>
</div>
<div class="stats-grid">
<div class="stat-card"><div class="stat-label">👥 کاربران</div><div class="stat-number" id="users">${stats.users}</div></div>
<div class="stat-card"><div class="stat-label">💬 پیام‌ها</div><div class="stat-number" id="messages">${stats.messages}</div></div>
<div class="stat-card"><div class="stat-label">🎙️ ویس‌ها</div><div class="stat-number" id="voices">${stats.voices}</div></div>
<div class="stat-card"><div class="stat-label">🎭 خلق‌وخو</div><div class="mood-display">${getMoodEmoji(sara.currentMood)}</div><div style="font-size:14px;opacity:0.8;">${sara.currentMood}</div></div>
</div>
<div class="controls">
...
<div class="logs" id="logs"><div class="log-entry">📋 آماده دریافت لاگ‌ها...</div></div>
</div>
<script>
const ADMIN_TOKEN='${env.ADMIN_TOKEN}';
async function controlBot(action){
try{
const r=await fetch('/admin/control',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+ADMIN_TOKEN},body:JSON.stringify({action})});
const result=await r.json();
addLog('✅ '+action+': '+result.message);
if(action==='status'&&result.stats){document.getElementById('users').textContent=result.stats.users;document.getElementById('messages').textContent=result.stats.messages;document.getElementById('voices').textContent=result.stats.voices}
if(action==='restart'||action==='start'||action==='stop'){setTimeout(()=>controlBot('status'),2000)}
}catch(e){addLog('❌ خطا: '+e.message)}}
function addLog(m){const l=document.getElementById('logs');const e=document.createElement('div');e.className='log-entry';e.textContent='['+new Date().toLocaleTimeString('fa-IR')+'] '+m;l.insertBefore(e,l.firstChild);while(l.children.length>50)l.removeChild(l.lastChild)}
setTimeout(()=>controlBot('status'),500);
setInterval(()=>controlBot('status'),30000);
</script>
</body>
</html>`;
...
async function handleAdminControl(action, env) {
  let message = '';
  switch (action) {
    case 'start':
      await startSelfBot(env);
    ...
    case 'status':
      const stats = await db.getStats();
      const status = await env.KV_NAMESPACE.get('bot_status') || 'stopped';
      return new Response(JSON.stringify({ status, stats, mood: sara.currentMood, managers: multiAccountManager.managers.length, activeUsers: userManager.getActiveUsers() }), { headers: { 'Content-Type': 'application/json' } });
    case 'clear_cache':
      memoryVault.memories.clear();
      message = 'کش پاک شد!';
      break;
...
export default {
  async fetch(request, env) {
    ...
    if (!multiAccountManager.managers.length) {
    ...
    if (path === '/webhook') {
      try {
        ...
        if (body.message) {
          await handleBotCommand(body, env);
          return new Response('OK', { status: 200 });
        }
        if (body.callback_query) {
          await handleAdminCallback(body.callback_query, env);
          return new Response('OK', { status: 200 });
        }
        return new Response('OK', { status: 200 });
      } catch (error) {
    ...
    if (path === '/admin/control') {
      ...
      const body = await request.json();
      return await handleAdminControl(body.action, env);
    }
    ...
    return new Response('🌸 سارا HYPER FUL - فعال', { status: 200 });
  },
