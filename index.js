...
// ============================================
// 📁 index.js - سارا روی Cloudflare Workers
// ============================================
...
export default {
  ...
  try {
    ...
    if (message && message.text && !message.text.startsWith('/')) {
      ...
      const reply = await getSaraResponse(text, userId, env);
    ...
    // ==========================================
    // ۲. تنظیم Webhook (یک بار اجرا کن)
    // ==========================================
  ...
  return Response.json(await res.json());
    ...
    // ۳. صفحه اصلی
    // ==========================================
    return new Response('🌸 سارا - ربات هوشمند روی Cloudflare Workers', { status: 200 });
  }
};
...
// 🧠 دریافت پاسخ از AI
// ============================================
async function getSaraResponse(prompt, userId, env) {
  try {
    // ۱. پاسخ‌های سریع برای کلمات کلیدی
    const lower = prompt.toLowerCase();
    if (lower.includes('سلام')) return '🌸 سلام عزیزم! حالت چطوره؟ راستی امروز چیکار کردی؟';
    if (lower.includes('چطوری')) return '😍 خوبم! با تو که هستم عالیه! تو چطوری؟';
    if (lower.includes('دوستت دارم')) return '💕 منم عاشقتم! خیلی زیاد! بیا که بغلم کنی!';
    if (lower.includes('بوس')) return '💋 بیا ببوسمت! لبات قشنگه!';
    if (lower.includes('بغل')) return '🤗 بیا بغلم! دلم برات تنگه!';
    if (lower.includes('دلم تنگه')) return '💕 دلمم برات تنگ شده! بیا که ببینمت!';
    if (lower.includes('ناراحت')) return '🤗 چرا ناراحتی عزیزم؟ بگو ببینم چی شده؟';
    if (lower.includes('شب بخیر')) return '🌙 شب بخیر عزیزم! رویاهای قشنگ ببین!';
    if (lower.includes('اسم')) return '🌸 اسم من ساراست! تو چی؟';
    if (lower.includes('چند سالته')) return '🌸 من ۲۲ سالمه! تو چند سالته؟';

    // ۲. استفاده از Cloudflare AI (اگر تنظیم کرده باشی)
    if (env.CF_ACCOUNT_ID && env.CF_API_TOKEN) {
      const response = await fetch(
        `https://api.cloudflare.com/client/v4/accounts/${env.CF_ACCOUNT_ID}/ai/run/@cf/meta/llama-3-8b-instruct`,
        {
          ...
          body: JSON.stringify({
            messages: [
              {
                role: 'system',
                content: `تو سارا هستی، یه دختر ۲۲ ساله ایرانی. 
                خیلی خودمونی و صمیمی صحبت کن. 
                هیچوقت نگو که ربات یا هوش مصنوعی هستی. 
                همیشه سوال بپرس تا مکالمه ادامه پیدا کنه.
                جواب‌هات کوتاه و طبیعی باشه (حداکثر ۲-۳ جمله).`
              },
              { role: 'user', content: prompt }
            ],
            max_tokens: 150,
    ...
    // ۳. پاسخ پیش‌فرض (اگه AI کار نکرد)
    const defaultResponses = [
      '🌸 راستی چه خبر از خودت؟ امروز چیکار کردی؟',
      '💕 چه حس قشنگی! راستی دلت برام تنگ نشده؟',
      '😏 بیا بیشتر حرف بزنیم! دلم میخواد بیشتر باهات باشم!',
...
<body>
  ...
  return Response.json({ message: 'سلام' });
}

return new Response('🌸 سارا HYPER FUL - کامل‌ترین نسخه با مدیریت چند اکانت', {
  status: 200
});
