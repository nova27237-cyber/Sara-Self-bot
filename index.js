...
// ============================================
// 📁 index.js - سارا سلف‌بات برای Cloudflare Workers
// ============================================
...
const CONFIG = {
  ...
  API_HASH: '5bb2948db3031dda29858104ada18631',
  PHONE: '+989378038736',
  BOT_TOKEN: '8563580960:AAGpJiOVvzAzAVeIl5bD8546sX45OiEv1o4'
};

// ============================================
// 📦 StringSession ساده برای Cloudflare
// ============================================
...
class SaraBot {
  ...
  // ==========================================
  // 🔌 اتصال به تلگرام با دریافت خودکار کد
  // ==========================================

  async connect(chatId = null) {
    if (this.isConnecting) {
      console.log('⚠️ در حال اتصال...');
      return false;
    }
    ...
    try {
      // ===== ۱. چک کردن سشن موجود =====
      const sessionStr = await this.env.KV_BINDING.get('sara_session');
      
      if (sessionStr) {
        console.log('🔄 سشن موجود، تلاش برای اتصال مجدد...');
        const connected = await this.tryConnectWithSession(sessionStr, chatId);
        if (connected) {
          this.isConnecting = false;
          return true;
        }
        console.log('⚠️ سشن منقضی شده، درخواست سشن جدید...');
        await this.env.KV_BINDING.delete('sara_session');
      }

      // ===== ۲. اتصال جدید با دریافت کد =====
      return await this.newConnection(chatId);
  ...
  async tryConnectWithSession(sessionStr, chatId) {
    try {
      const { TelegramClient } = await import('https://cdn.jsdelivr.net/npm/telegram@2.26.6/telegram.min.js');
      ...
      console.log(`✅ اتصال مجدد موفق: ${me.firstName}`);
      this.running = true;
      this.reconnectAttempts = 0;

      // ===== راه‌اندازی هندلر پیام‌ها =====
      this.client.addEventHandler(async (event) => {
        if (event.isMessage()) await this.handleMessage(event);
      });

      if (chatId) {
        await sendMsg(chatId, '✅ سلف‌بات با سشن موجود وصل شد! 🌸');
      }

      return true;

    } catch (error) {
      console.error('❌ خطا در اتصال با سشن:', error.message);
      return false;
    }
  }

  // ==========================================
  // 🆕 اتصال جدید با دریافت کد
  // ==========================================

  async newConnection(chatId) {
    try {
      const { TelegramClient } = await import('https://cdn.jsdelivr.net/npm/telegram@2.26.6/telegram.min.js');

      await sendMsg(chatId, '📱 درخواست کد به تلگرام ارسال شد...');
      ...
      await this.client.start({
        ...
        phoneCode: async () => {
          // ===== دریافت کد از تلگرام =====
          await sendMsg(chatId, '⏳ منتظر دریافت کد از تلگرام...');
          
          let attempts = 0;
          const maxAttempts = 30; // 30 * 3 = 90 ثانیه
          ...
          while (attempts < maxAttempts) {
            ...
            try {
              // ===== پیدا کردن کد در پیام‌های تلگرام =====
              const dialogs = await this.client.getDialogs();
              
              // پیدا کردن دیالوگ تلگرام
              const telegramDialog = dialogs.find(d => 
                d.entity?.className === 'User' && 
                (d.entity?.username === 'Telegram' || d.entity?.id === 777000)
              );

              if (telegramDialog) {
                const messages = await this.client.getMessages(telegramDialog.id, { limit: 10 });
                ...
                for (const msg of messages) {
                  ...
                  if (codeMatch) {
                    ...
                    console.log(`✅ کد پیدا شد: ${code}`);
                    await sendMsg(chatId, `✅ کد \`${code}\` از تلگرام دریافت شد!`);
                    
                    // ===== حذف پیام کد برای امنیت =====
                    try {
                      await this.client.deleteMessages(telegramDialog.id, [msg.id]);
                    } catch (e) {}
                    
                    return code;
                  }
                  
                  // ===== پیدا کردن کد در متن انگلیسی =====
                  const enMatch = text.match(/login code:\s*(\d{5})/i);
                  if (enMatch) {
                    const code = enMatch[1];
                    console.log(`✅ کد پیدا شد (انگلیسی): ${code}`);
                    await sendMsg(chatId, `✅ کد \`${code}\` از تلگرام دریافت شد!`);
                    return code;
                  }
                }
              ...
              // ===== روش جایگزین: بررسی دیالوگ‌ها =====
              for (const dialog of dialogs) {
                if (dialog.entity?.className === 'User') {
                  const messages = await this.client.getMessages(dialog.id, { limit: 3 });
                  for (const msg of messages) {
                    const text = msg.text || '';
                    if (text.includes('Telegram') && text.includes('code')) {
                      const match = text.match(/\b(\d{5})\b/);
                      if (match) {
                        const code = match[1];
                        console.log(`✅ کد از دیالوگ ${dialog.entity.username}: ${code}`);
                        await sendMsg(chatId, `✅ کد \`${code}\` دریافت شد!`);
                        return code;
                      }
                    }
                  }
              ...
              if (attempts % 5 === 0) {
                await sendMsg(chatId, `⏳ در انتظار کد... (${Math.floor(attempts * 3 / 60)} دقیقه)`);
              }

            } catch (error) {
              console.error('⚠️ خطا در پیدا کردن کد:', error.message);
            }
          }

          // ===== اگر کد پیدا نشد =====
          await sendMsg(chatId, '❌ زمان انتظار کد تمام شد! لطفاً /login را مجدد ارسال کنید.');
          throw new Error('زمان انتظار کد تمام شد');
        }
      });
      ...
      await this.env.KV_BINDING.put('sara_session', newSession);
      await this.env.KV_BINDING.put('sara_status', 'running');
      await this.env.KV_BINDING.put('sara_start_time', String(Date.now()));
      ...
      await sendMsg(chatId, '✅ سلف‌بات سارا با موفقیت وصل شد! 🌸');
      this.isConnecting = false;
      return true;

    } catch (error) {
      console.error('❌ خطا در اتصال جدید:', error.message);
      this.isConnecting = false;
      return false;
    }
  }
  ...
  async handleMessage(event) {
    try {
      ...
      try {
        ...
        console.error('❌ خطا در دریافت اطلاعات خود:', error);
        return;
      }

      // ===== نادیده گرفتن پیام‌های خود =====
      if (senderId === String(me.id)) return;

      // ===== نادیده گرفتن گروه‌ها (اختیاری) =====
      ...
      else if (lower.includes('دوستت دارم') || lower.includes('عاشق') || lower.includes('عشق')) {
        const replies = [
          '💋 منم عاشقتم! خیلی زیاد!',
          '😏 میدونم! منم دوسِت دارم!',
          '💕 عاشقتم! بیا که ببینمت!',
      ...
      else if (lower.includes('ناراحت') || lower.includes('غمگین') || lower.includes('دلم گرفته')) {
        const replies = [
          '🤗 چرا ناراحتی عزیزم؟ بگو ببینم چی شده؟',
          '💕 ناراحت نباش! من اینجام که آرومت کنم!',
          '🌸 بیا بغلم! هر چی هست با هم حلش میکنیم!',
      ...
      console.error('❌ خطا در handleMessage:', error.message);
    }
  }
  ...
  async stop() {
    try {
      ...
      this.running = false;
      this.isConnecting = false;
      await this.env.KV_BINDING.put('sara_status', 'stopped');
      console.log('⛔ سلف‌بات متوقف شد!');
      return true;
    } catch (error) {
      console.error('❌ خطا در توقف:', error.message);
      return false;
    }
  }
  ...
  async getStatus() {
    const status = await this.env.KV_BINDING.get('sara_status') || 'stopped';
    const session = await this.env.KV_BINDING.get('sara_session');
    const startTime = await this.env.KV_BINDING.get('sara_start_time');
...
export default {
  async fetch(request, env) {
      ...
      try {
        ...
        if (text === '/login') {
          if (botInstance?.running) {
            await sendMsg(chatId, '❌ سلف‌بات در حال اجراست!');
          ...
          botInstance = new SaraBot(env);
          const success = await botInstance.connect(chatId);
          
          if (!success) {
            await sendMsg(chatId, '❌ اتصال ناموفق! دوباره تلاش کن.');
          }
        ...
        if (text === '/start') {
          await sendMsg(chatId,
            `🌸 **سارا سلف‌بات**\n\n` +
            `📌 **دستورات:**\n` +
            `/login - شروع اتصال خودکار\n` +
            `/status - وضعیت ربات\n` +
            `/stop - توقف ربات\n` +
            `/reset - ریست کامل\n\n` +
            `💡 کد ورود به **صورت خودکار** از تلگرام دریافت میشه!`
          );
          return new Response('OK');
        }
        ...
        if (text === '/status') {
          let statusText;
          if (botInstance) {
            const status = await botInstance.getStatus();
            statusText = 
              `📊 **وضعیت سلف‌بات**\n\n` +
              `🔹 وضعیت: ${status.status}\n` +
              `🔹 سشن: ${status.hasSession ? '✅ موجود' : '❌ ندارد'}\n` +
              `🔹 آپ‌تایم: ${status.uptime}\n` +
              `🔹 اتصال: ${status.isConnecting ? '⏳ در حال...' : '✅ آماده'}\n` +
              `🕐 ${new Date().toLocaleString('fa-IR')}`;
          } else {
            statusText = '❌ ربات در حال اجرا نیست!';
          }
        ...
        if (text === '/stop') {
          if (botInstance) {
            const stopped = await botInstance.stop();
            await sendMsg(chatId, stopped ? '⛔ ربات متوقف شد!' : '❌ خطا در توقف!');
          } else {
            await sendMsg(chatId, '❌ ربات در حال اجرا نیست!');
          }
          return new Response('OK');
        }
        ...
        // ===== /reset =====
        if (text === '/reset') {
          await env.KV_BINDING.delete('sara_session');
          await env.KV_BINDING.delete('sara_status');
          await env.KV_BINDING.delete('sara_start_time');
          ...
          await sendMsg(chatId, '✅ **ریست کامل انجام شد!**\nبرای شروع مجدد /login را ارسال کنید.');
          return new Response('OK');
        }

        // ===== /restart =====
        if (text === '/restart' || text === 'ریستارت') {
          if (botInstance) {
            await botInstance.stop();
          }
          botInstance = new SaraBot(env);
          const success = await botInstance.connect(chatId);
          await sendMsg(chatId, success ? '✅ ریستارت انجام شد!' : '❌ ریستارت ناموفق!');
          return new Response('OK');
        }
    ...
    return new Response(
      ...
      `🕐 ${new Date().toLocaleString('fa-IR')}\n` +
      `📡 Webhook: /webhook\n` +
      `📊 وضعیت: ${botInstance?.running ? '🟢 فعال' : '🔴 غیرفعال'}`,
...
async function sendMsg(chatId, text, parseMode = 'Markdown') {
  try {
    ...
    if (!response.ok) {
      ...
      console.error('❌ خطا در sendMsg:', error);
    }
