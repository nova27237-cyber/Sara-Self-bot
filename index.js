  ...
  async fetch(request, env) {
    ...
    if (url.pathname === '/setcode') {
      ...
      if (code) {
        ...
        return new Response('✅ کد دریافت شد! حالا /start رو بزن');
      }
    ...
    if (url.pathname === '/status') {
      const status = await env.KV_BINDING.get('bot_status') || 'stopped';
      return new Response(`📊 وضعیت: ${status}`);
    }
    
    // ======== صفحه اصلی ========
    return new Response(`
🌸 سارا سلف‌بات - نسخه نهایی

📌 دستورات:
• /setcode?code=XXXXX - دریافت کد تایید
• /start - شروع سلف‌بات
• /stop - توقف سلف‌بات
• /status - وضعیت

📱 بعد از شروع، تو تلگرام بفرست:
.ai سلام چطوری؟

⚠️ هشدار: سلف‌بات ریسک بن شدن داره!
    `, {
      status: 200,
