...
async function handleSelfMessage(event, client, env) {
  try {
    ...
    if (text === '/start') {
      await client.sendMessage(chatId, { 
        message: langManager.translate('start', langManager.getUserLang(userId), message.sender?.firstName || 'عزیزم')
      });
      return;
    }
    ...
    if (text === '/help' || text.toLowerCase() === 'help') {
      await client.sendMessage(chatId, { 
        message: langManager.translate('help', langManager.getUserLang(userId))
      });
      return;
    }
    ...
    if (text === '/status') {
      ...
      await client.sendMessage(chatId, {
        message: langManager.translate('status', langManager.getUserLang(userId),
    ...
    if (text === '/profile') {
      ...
      await client.sendMessage(chatId, {
        message: langManager.translate('profile', langManager.getUserLang(userId),
          me.firstName || 'سارا',
    ...
    if (text === '/ip') {
      ...
      await client.sendMessage(chatId, {
        message: langManager.translate('ip_hash', langManager.getUserLang(userId), ipHash || 'تنظیم نشده')
      });
      return;
    }
    ...
    if (adminIds.includes(parseInt(senderId.toString()))) {
      if (text === '/stop') {
        ...
        await client.sendMessage(chatId, {
          message: langManager.translate('bot_stopped', langManager.getUserLang(userId))
        });
        return;
      }
      ...
      if (text === '/restart') {
        await client.sendMessage(chatId, { 
          message: langManager.translate('restarting', langManager.getUserLang(userId))
        });
        await stopSelfBot(env);
        await new Promise(resolve => setTimeout(resolve, 3000));
        await startSelfBot(env);
        await client.sendMessage(chatId, { 
          message: langManager.translate('restart_done', langManager.getUserLang(userId))
        });
        return;
      }
      ...
      if (text === '/clear_cache') {
        ...
        await client.sendMessage(chatId, { 
          message: langManager.translate('cache_cleared', langManager.getUserLang(userId))
        });
        return;
      }
      ...
      if (text.startsWith('/broadcast ')) {
        ...
        if (msg) {
          ...
          await client.sendMessage(chatId, { 
            message: langManager.translate('broadcast_sent', langManager.getUserLang(userId), 'همه')
          });
        }
        return;
      }
    ...
    if (broadcastState === 'waiting' && text !== '/cancel') {
      ...
      await client.sendMessage(chatId, {
        message: langManager.translate('broadcast_sent', langManager.getUserLang(userId), successCount)
      });
      return;
    }
    ...
    if (text === '/cancel') {
      await env.KV_BINDING.delete(`broadcast:${chatId}`);
      await client.sendMessage(chatId, { 
        message: langManager.translate('broadcast_cancel', langManager.getUserLang(userId))
      });
      return;
    }
...
async function showUsersList(chatId, client, env, userId) {
  try {
    ...
    await client.sendMessage(chatId, {
      message: langManager.translate('user_list', langManager.getUserLang(userId), sorted.length, userList)
    });
  } catch (error) {
...
async function showAdminPanel(chatId, messageId, client, env, userId) {
  try {
    ...
    await client.sendMessage(chatId, {
      message: langManager.translate('panel', langManager.getUserLang(userId),
        stats.users,
        stats.messages,
        stats.voices,
        '1',
...
async function showLanguageMenu(chatId, env, userId) {
  ...
  await client.sendMessage(chatId, {
    message: '🌐 **انتخاب زبان / Choose Language:**',
    buttons: keyboard,
    parseMode: 'Markdown'
  });
}
  ...
  async fetch(request, env) {
    try {
      ...
      const envData = {
        ...
        totalVoices,
        botStatus,
        uptime: startTime ? await getUptime(env) : 'نامشخص'
      };
