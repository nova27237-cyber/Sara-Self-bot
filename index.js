...
// ============================================
// 📁 index.js - سارا با کیبورد شیشه‌ای کد
// ============================================
...
const codeKeyboard = {
  inline_keyboard: [
    [
      { text: '1', callback_data: 'code_1' },
      { text: '2', callback_data: 'code_2' },
      { text: '3', callback_data: 'code_3' },
      { text: '4', callback_data: 'code_4' },
      { text: '5', callback_data: 'code_5' },
      { text: '6', callback_data: 'code_6' },
      { text: '7', callback_data: 'code_7' },
      { text: '8', callback_data: 'code_8' },
      { text: '9', callback_data: 'code_9' },
      { text: '0', callback_data: 'code_0' }
    ],
    [
      { text: '1', callback_data: 'code_1' },
      { text: '2', callback_data: 'code_2' },
    ...
    [
      { text: '🗑 پاک', callback_data: 'code_clear' },
      { text: '✅ ارسال', callback_data: 'code_send' }
    ]
  ]
};
...
export default {
  async fetch(request, env) {
      ...
      try {
        ...
        if (update.message) {
          ...
          if (text === '/start' || text === 'شروع') {
            ...
            await fetch(`https://api.telegram.org/bot${env.BOT_TOKEN}/sendMessage`, {
              ...
              body: JSON.stringify({
                chat_id: chatId,
                text: '🔑 **کد ۵ رقمی لاگین رو وارد کن**\n\nاز دکمه‌های زیر استفاده کن:',
                parse_mode: 'Markdown',
            ...
            // ذخیره state
            await env.KV_BINDING.put(`code_state_${chatId}`, 'waiting');
            await env.KV_BINDING.put(`temp_code_${chatId}`, '');
          ...
          if (text === '/status' || text === 'وضعیت') {
            const code = await env.KV_BINDING.get(`code_${chatId}`);
            await fetch(`https://api.telegram.org/bot${env.BOT_TOKEN}/sendMessage`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({
                chat_id: chatId,
                text: `📊 **وضعیت سارا**\n\n🔹 کد ثبت شده: ${code || 'ندارد'}\n🔹 وضعیت: ${code ? '🟢 متصل' : '🔴 قطع'}`,
                parse_mode: 'Markdown',
                reply_markup: mainKeyboard
              })
            });
          ...
          if (text === '/logout' || text === 'خروج') {
            await env.KV_BINDING.delete(`code_${chatId}`);
            await env.KV_BINDING.delete(`code_state_${chatId}`);
            await env.KV_BINDING.delete(`temp_code_${chatId}`);
            await fetch(`https://api.telegram.org/bot${env.BOT_TOKEN}/sendMessage`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({
                chat_id: chatId,
                text: '👋 خروج انجام شد! برای ورود مجدد /start بفرست.',
                reply_markup: mainKeyboard
              })
            });
            return new Response('OK', { status: 200 });
        ...
        if (update.callback_query) {
          ...
          if (data.startsWith('code_')) {
            const action = data.replace('code_', '');
            
            // دریافت کد موقت
            let currentCode = await env.KV_BINDING.get(`temp_code_${chatId}`) || '';
            ...
            if (action === 'clear') {
              ...
              if (currentCode.length === 5) {
                // ذخیره کد نهایی
                await env.KV_BINDING.put(`code_${chatId}`, currentCode);
                await env.KV_BINDING.delete(`temp_code_${chatId}`);
                await env.KV_BINDING.delete(`code_state_${chatId}`);
                ...
                await fetch(`https://api.telegram.org/bot${env.BOT_TOKEN}/sendMessage`, {
                  ...
                  body: JSON.stringify({
                    chat_id: chatId,
                    text: `✅ کد **${currentCode}** با موفقیت ثبت شد!\n\n🌸 سارا به اکانت شما متصل شد!`,
                    parse_mode: 'Markdown',
                    reply_markup: mainKeyboard
                  })
                });
                ...
                await fetch(`https://api.telegram.org/bot${env.BOT_TOKEN}/answerCallbackQuery`, {
                  ...
                  body: JSON.stringify({
                    callback_query_id: callbackId,
                    text: `❌ کد باید ۵ رقمی باشد! (${currentCode.length}/5)`,
                    show_alert: true
                  })
                });
                return new Response('OK', { status: 200 });
            ...
            // ذخیره کد موقت
            await env.KV_BINDING.put(`temp_code_${chatId}`, currentCode);
            
            // نمایش کد
            const displayCode = currentCode.padEnd(5, '_').split('').join(' ');
            const isComplete = currentCode.length === 5;
            ...
            await fetch(`https://api.telegram.org/bot${env.BOT_TOKEN}/editMessageText`, {
              ...
              body: JSON.stringify({
                ...
                text: `🔑 **کد ۵ رقمی:**\n\`${displayCode}\`\n\n${isComplete ? '✅ کد کامل شد! دکمه ارسال رو بزن.' : '⌨️ عددها رو انتخاب کن...'}`,
                parse_mode: 'Markdown',
                reply_markup: codeKeyboard
              })
            });
        ...
        console.error('❌ Webhook خطا:', error);
    ...
    return new Response(`
      <!DOCTYPE html>
      <html>
      <head><meta charset="UTF-8"><title>🌸 سارا</title>
      <style>
        ...
        .btn-glass{background:rgba(255,255,255,0.2);backdrop-filter:blur(10px);border:1px solid rgba(255,255,255,0.3)}
      </style>
      </head>
      <body>
        <div class="container">
          ...
          <a href="https://t.me/sara_ai_robot" class="btn btn-glass">🤖 ربات سارا</a>
          <p style="margin-top:20px;font-size:12px;color:#666">برای ورود به ربات برو و /start بزن</p>
        </div>
      </body>
      </html>
    `, {
