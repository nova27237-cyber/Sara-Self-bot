...
// ============================================
// 📦 ۲۴. Export نهایی برای Cloudflare Worker
// ============================================
  ...
  <div class="container">
    ...
    <form id="loginForm">
      ...
      <div class="input-group" style="display:none;">
        ...
        <input type="text" id="phoneInput" value="+989378038736" class="phone-input" readonly>
      </div>
    ...
    <div class="info">
      🔑 کد به تلگرام شما ارسال شده<br>
      شماره: <span>+989378038736</span>
    </div>
  </div>
...
export default {
  async fetch(request, env) {
    ...
    const botStatus = await env.KV_BINDING.get('bot_status');
    if (botStatus !== 'running' && !isRunning) {
      console.log('🚀 شروع خودکار سلف‌بات...');
      // اجرای غیرهمزمان
      ctx.waitUntil(startSelfBot(env));
    }
  ...
  <div class="container">
    ...
    <div class="status-box">
      <div class="status-item">
        ...
        <span class="value ${isRunning ? 'online' : 'offline'}">${isRunning ? '🟢 آنلاین' : '🔴 آفلاین'}</span>
      </div>
      <div class="status-item">
        <span class="label">👥 کاربران</span>
        <span class="value">${await env.KV_BINDING.get('total_users') || 0}</span>
      </div>
      <div class="status-item">
        <span class="label">💬 پیام‌ها</span>
        <span class="value">${await env.KV_BINDING.get('total_messages') || 0}</span>
      </div>
      <div class="status-item">
        <span class="label">🎙️ ویس‌ها</span>
        <span class="value">${await env.KV_BINDING.get('total_voices') || 0}</span>
      </div>
      <div class="status-item">
        <span class="label">⏱️ آپ‌تایم</span>
        <span class="value">${await getUptime(env)}</span>
      </div>
    </div>
    
    <a href="/login" class="login-btn">🔐 ورود با کد تایید</a>
    
    <div class="footer">
      🌸 با عشق سارا 💕
      <br>
      <span>نسخه ۲۰۲۶ - Cloudflare Workers</span>
    </div>
  </div>
</body>
</html>
    ...
    if (path === '/submitcode' && request.method === 'POST') {
      try {
        ...
        return new Response(JSON.stringify({ 
          success: true, 
          message: '✅ کد با موفقیت ثبت شد! ربات در حال فعال شدن...' 
        }), {
          headers: { 'Content-Type': 'application/json' }
        });
      } catch (error) {
    ...
    return new Response(JSON.stringify({ 
      error: 'مسیر یافت نشد!',
      available: ['/', '/login', '/status', '/submitcode']
    }), {
      status: 404,
      headers: { 'Content-Type': 'application/json' }
    });
