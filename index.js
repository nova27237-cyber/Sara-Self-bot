...
// ============================================
// سلف‌بات جاوااسکریپت - بخش لاگین و کد تایید
// ============================================
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .info span { color: #764ba2; font-weight: bold; }
    input {
      width: 100%;
      padding: 14px;
      font-size: 22px;
...
<body>
  <div class="container">
    ...
    <div class="info">
      📱 شماره: <span>${CONFIG.PHONE_NUMBER}</span>
    </div>
    ...
    <div class="loading" id="loading">
      ...
      <p style="margin-top:10px;color:#764ba2;">در حال ثبت کد...</p>
    </div>

    <div class="footer">🔑 کد به تلگرام شما ارسال شده</div>
  </div>
...
async function handleSubmitCode(request, env) {
  try {
    const body = await request.json();
    const code = body.code;
    const phone = body.phone || CONFIG.PHONE_NUMBER;
    ...
    // شروع ربات بعد از ثبت کد
    // تابع startBot رو اینجا صدا بزن
...
<body>
  <div class="container">
    <div class="logo">🌸</div>
    <h1>سلف‌بات</h1>
    <p class="subtitle">💕 ورود به اکانت تلگرام</p>

    <div class="status">
      <p>📱 شماره: <strong>${CONFIG.PHONE_NUMBER}</strong></p>
      <p>🔄 وضعیت: <span class="online" id="statusText">در حال بررسی...</span></p>
    </div>

    <a href="/login" class="btn">🔐 ورود با کد</a>

    <div class="footer">🌸 با عشق</div>
  </div>
  ...
  <script>
    ...
    if (path === '/submitcode' && request.method === 'POST') {
      return await handleSubmitCode(request, env);
    }
