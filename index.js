...
// ============================================
// 📁 index.js - دریافت خودکار کد از تلگرام
// ============================================
...
class SaraBot {
  ...
  // ===== شروع با دریافت خودکار کد =====
  async start(chatId) {
    try {
      await this.env.KV_BINDING.delete('session');
      ...
      await this.client.start({
        ...
        password: async () => '',
        phoneCode: async () => {
          // ===== دریافت کد از پیام‌های تلگرام =====
          await sendMsg(chatId, '⏳ منتظر دریافت کد از تلگرام...');
          
          // تابع برای پیدا کردن کد در پیام‌ها
          const findCode = async () => {
            try {
              ...
              const telegramDialog = dialogs.find(d => 
                ...
                d.entity?.username === 'Telegram'
              );

              if (telegramDialog) {
                const messages = await this.client.getMessages(telegramDialog.id, { limit: 5 });
              ...
              // اگر کد پیدا نشد، صبر کن و دوباره تلاش کن
              await new Promise(r => setTimeout(r, 3000));
              return await findCode();
              
            } catch (error) {
              console.error('❌ خطا در پیدا کردن کد:', error);
              await new Promise(r => setTimeout(r, 3000));
              return await findCode();
            }
          };
      ...
      await this.env.KV_BINDING.put('session', sessionStr);
      this.running = true;

      // هندلر پیام‌ها
      this.client.addEventHandler(async (e) => {
        if (e.isMessage()) await this.handleMessage(e);
      });

      await sendMsg(chatId, '✅ سلف‌بات وصل شد! 🌸');
      return true;

    } catch (error) {
      console.error('❌ خطا:', error.message);
      await sendMsg(chatId, `❌ خطا: ${error.message}`);
      return false;
    }
  }
  ...
  async resume() {
    try {
      const sessionStr = await this.env.KV_BINDING.get('session');
  ...
  async handleMessage(event) {
    try {
      ...
      if (sender === String(me.id)) return;
      if (String(chat).startsWith('-')) return;

      await this.client.deleteMessages(chat, [msg.id]);
      ...
      await this.client.sendMessage(chat, { message: reply });
    } catch (error) {
  ...
  async stop() {
    ...
    this.running = false;
    await this.env.KV_BINDING.put('status', 'stopped');
  }
}
...
export default {
  async fetch(request, env) {
      ...
      try {
        ...
        if (text === '/start') {
          await sendMsg(chatId,
            ...
            `/stop - توقف\n` +
            `/reset - ریست\n\n` +
            `📌 کد به طور خودکار از تلگرام دریافت میشه!`
          );
          return new Response('OK');
        }
        ...
        // ===== /status =====
        if (text === '/status') {
          const status = bot?.running ? '🟢 آنلاین' : '🔴 آفلاین';
          const session = await env.KV_BINDING.get('session');
          
          await sendMsg(chatId,
            `📊 وضعیت:\n` +
            `🔹 سلف‌بات: ${status}\n` +
            `🔹 سشن: ${session ? '✅ موجود' : '❌ ندارد'}\n` +
            `🕐 ${new Date().toLocaleString('fa-IR')}`
          );
          return new Response('OK');
        }
        ...
        if (text === '/stop') {
          if (bot) {
            await bot.stop();
            bot = null;
            await sendMsg(chatId, '⛔ متوقف شد!');
          } else {
            await sendMsg(chatId, '❌ در حال اجرا نیست!');
          }
          return new Response('OK');
        }
        ...
        // ===== /reset =====
        if (text === '/reset') {
          await env.KV_BINDING.delete('session');
          if (bot) {
            await bot.stop();
            bot = null;
          }
          await sendMsg(chatId, '✅ ریست شد!');
          return new Response('OK');
        }
    ...
    return new Response(`🌸 سارا آنلاین!\n${new Date().toLocaleString('fa-IR')}`);
  }
};
