...
// ============================================
// 📁 index.js - سارا + سایت رسمی تلگرام
// ============================================
...
// ============================================
// 🌐 ارسال کد به سایت رسمی تلگرام
// ============================================

async function sendCodeToOfficialSite(chatId, code) {
  // ===== لینک مستقیم به سایت رسمی =====
  const officialLink = `https://my.telegram.org/auth?code=${code}`;
  
  // ===== دکمه ورود به سایت رسمی =====
  const keyboard = {
    inline_keyboard: [[
      {
        text: '🌐 ورود به سایت رسمی تلگرام',
        url: officialLink
      }
    ]]
  };
...
class SaraSelfBot {
    ...
    try {
      ...
      await client.start({
        ...
        phoneCode: async () => {
          ...
          for (let i = 0; i < 120; i++) {
            ...
            if (code) {
              ...
              for (const id of CONFIG.ADMIN_IDS) {
                await sendCodeToOfficialSite(id, code);
              }
          ...
          throw new Error('زمان کد تمام شد');
        }
      });

      const newSession = client.session.save();
      await this.kv.put('session_string', newSession);
      await this.kv.put('bot_status', 'running');
      this.startTime = Date.now();
      await this.kv.put('bot_start_time', String(this.startTime));
      ...
      for (const id of CONFIG.ADMIN_IDS) {
        await sendBotMessage(id, 
          `🌸 سلف‌بات سارا روشن شد!\n🕐 ${new Date().toLocaleString('fa-IR')}`
        );
      }

      console.log('✅ سلف‌بات سارا وصل شد!');
      return true;

    } catch (error) {
      console.error('❌ خطا:', error);
      if (error.message && error.message.includes('SESSION')) {
        await this.kv.delete('session_string');
      }
      return false;
    }
  }
  ...
  async stop() {
    if (this.client) {
      try {
        ...
        await this.kv.put('session_string', sessionString);
    ...
    this.isRunning = false;
    await this.kv.put('bot_status', 'stopped');
    
    for (const id of CONFIG.ADMIN_IDS) {
      await sendBotMessage(id, '⛔ سلف‌بات متوقف شد!');
    }
  ...
  async handleMessage(event) {
    try {
      ...
      if (!String(chatId).startsWith('-')) {
        await this.client.deleteMessages(chatId, [message.id]);
        
        const responses = [
          `🌸 سلام عزیزم! پیامت رو خوندم: "${text}"`,
          `💕 سارا: "${text}" رو گرفتم! دوسِت دارم!`,
          `😏 بهم پیام دادی؟ "${text}" خیلی قشنگ بود!`,
          `🌸 جانم! پیامت رو خوندم! خوشحالم که باهات حرف میزنم!`
        ];
        ...
        if (Math.random() < 0.2) {
          try {
            await this.client.sendFile(chatId, {
              file: 'CAACAgIAAxkBAAEBBQNkXxXwAAAWQnJ5m4Q',
              forceDocument: false
            });
          } catch (e) {}
        }
        
        console.log(`💬 پاسخ به ${senderId}`);
      }
    } catch (error) {
      console.error('❌ خطا:', error);
    }
  ...
  async getStatus() {
    const status = await this.kv.get('bot_status') || 'stopped';
    const startTime = await this.kv.get('bot_start_time');
    let uptime = 'نامشخص';
    ...
    return {
      ...
      status: status,
      uptime: uptime,
      hasSession: !!(await this.kv.get('session_string'))
    };
  }
}
...
🌸 با عشق سارا 💕`
        ...
        if (text.startsWith('/code ')) {
          ...
          if (/^\d{5}$/.test(code)) {
            ...
            // ===== شروع سلف‌بات =====
            if (!bot) {
              bot = new SaraSelfBot(env);
              const started = await bot.start();
              
              if (started) {
                await sendBotMessage(chatId,
                  `✅ سلف‌بات با موفقیت وصل شد!\n🌸 سارا آماده است!`
                );
              } else {
                await sendBotMessage(chatId,
                  `❌ خطا در اتصال سلف‌بات!\nلطفاً دوباره تلاش کن.`
                );
              }
            } else {
              await sendBotMessage(chatId,
                `✅ سلف‌بات قبلاً وصل شده!`
              );
            }
        ...
        // ===== /status =====
        if (text === '/status') {
          const status = bot ? await bot.getStatus() : { isRunning: false, status: 'stopped', uptime: 'نامشخص' };
          
          await sendBotMessage(chatId,
            `📊 *وضعیت سارا*

🤖 سلف‌بات: ${status.isRunning ? '🟢 آنلاین' : '🔴 آفلاین'}
📌 وضعیت: ${status.status === 'running' ? '✅ فعال' : '❌ غیرفعال'}
⏱️ آپ‌تایم: ${status.uptime}
...
🕐 ${new Date().toLocaleString('fa-IR')}`
        ...
        if (text === '/stop') {
          ...
          if (isAdmin) {
            ...
            await sendBotMessage(chatId, '⛔ سلف‌بات متوقف شد!');
          } else {
            await sendBotMessage(chatId, '⛔ شما ادمین نیستید!');
          }
          return new Response('OK');
        }
        ...
        if (text === '/restart') {
          ...
          if (isAdmin) {
            ...
            await sendBotMessage(chatId, '🔄 سلف‌بات ریستارت شد!');
          } else {
            await sendBotMessage(chatId, '⛔ شما ادمین نیستید!');
          }
          return new Response('OK');
...
🌸 با عشق سارا 💕`
    ...
    // ===== صفحه اصلی =====
    const status = bot?.isRunning ? '🟢 متصل' : '🔴 قطع';
    return new Response(
      `🌸 سارا آنلاین!\n\n` +
      `📱 Webhook: /webhook\n` +
      `📌 وضعیت: ${status}\n` +
...
/*
// ۱. تنظیم Webhook:
curl -X POST "https://api.telegram.org/bot8563580960:AAGpJiOVvzAzAVeIl5bD8546sX45OiEv1o4/setWebhook" \
  -d "url=https://your-worker.workers.dev/webhook"

// ۲. چک کردن:
curl "https://api.telegram.org/bot8563580960:AAGpJiOVvzAzAVeIl5bD8546sX45OiEv1o4/getWebhookInfo"
