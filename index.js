...
// ============================================
// 📸 سارا - کامل با عکس، ویدیو، تصویری و تماس
// ============================================
...
function getNaturalResponse(text) {
  ...
  if (lower.includes('دوستت دارم') || lower.includes('عاشق')) {
    ...
    if (url.pathname === '/webhook') {
      try {
        ...
        if (callback) {
          ...
          if (callback.data === 'photo') await sendSaraPhoto(chatId, 'photo');
          else if (callback.data === 'video') await sendSaraVideo(chatId);
          else if (callback.data === 'art') await sendSaraArt(chatId);
          else if (callback.data === 'continue') {
            const reply = getNaturalResponse('ادامه بده');
            await fetch(`https://api.telegram.org/bot${BOT_TOKEN}/sendMessage`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ chat_id: chatId, text: `📹 ${reply}` })
            });
          } else if (callback.data === 'end' || callback.data === 'close') {
        ...
        if (message && message.text) {
          ...
          if (command === 'photo' || command === 'home' || command === 'formal' || command === 'party' || command === 'sport') {
            await sendSaraPhoto(chatId, command);
            return new Response('OK');
          }
          if (command === 'video') {
            await sendSaraVideo(chatId);
            return new Response('OK');
          }
          if (command === 'art') {
            await sendSaraArt(chatId);
            return new Response('OK');
          }
          
          if (text === '/call' || text === '/videocall') {
            const reply = getNaturalResponse('سلام');
            await fetch(`https://api.telegram.org/bot${BOT_TOKEN}/sendMessage`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({
                ...
                reply_markup: {
                  inline_keyboard: [
                    [{ text: '📹 ادامه', callback_data: 'continue' }],
                    [{ text: '📸 عکس', callback_data: 'photo' }],
                    [{ text: '🎬 ویدیو', callback_data: 'video' }],
                    [{ text: '❌ قطع', callback_data: 'end' }]
                  ]
                }
              })
            });
...
🎬 ویدیو بفرست - ویدیوی سارا
🎨 تصویری بفرست - عکس هنری
👗 با مانتو بفرست - عکس با لباس خاص
...
💕 سارا عاشق توست!`
          ...
          const reply = getNaturalResponse(text);
          await fetch(`https://api.telegram.org/bot${BOT_TOKEN}/sendMessage`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ chat_id: chatId, text: `🌸 ${reply}` })
    ...
    return new Response('🌸 سارا - عکس، ویدیو، تصویری و تماس');
  }
};
