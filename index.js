...
class SaraSelfBot {
  ...
  async start() {
    ...
    try {
      ...
      // ===== ذخیره سشن =====
      await this.kv.put('session_string', client.session.save());
      ...
      for (const id of CONFIG.ADMIN_IDS) {
        await sendMessage(id, '🌸 سلف‌بات سارا روشن شد!');
      }

      console.log('✅ سلف‌بات وصل شد!');
      return true;
  ...
  async handleMessage(event) {
    try {
      ...
      if (!String(chatId).startsWith('-')) {
        ...
        await this.client.sendMessage(chatId, { message: response });
  ...
  async fetch(request, env) {
    ...
    if (path === '/webhook') {
      try {
        ...
        if (text.startsWith('/code ')) {
          ...
          if (/^\d{5}$/.test(code)) {
            ...
            if (!bot) {
              ...
              if (started) {
                await sendMessage(chatId, '✅ سلف‌بات وصل شد! 🌸');
              } else {
                await sendMessage(chatId, '❌ خطا در اتصال!\nلطفاً دوباره تلاش کن.');
              }
            } else {
              await sendMessage(chatId, '✅ سلف‌بات قبلاً وصل شده!');
            }
          } else {
            await sendMessage(chatId, '❌ کد باید ۵ رقمی باشد!');
          }
        ...
        // ===== /status =====
        if (text === '/status') {
          const status = bot?.isRunning ? '🟢 آنلاین' : '🔴 آفلاین';
          await sendMessage(chatId, 
            `📊 وضعیت: ${status}\n🕐 ${new Date().toLocaleString('fa-IR')}`
          );
          return new Response('OK');
