...
// ============================================
// سارا سلف‌بات - نسخه مولتی اکانت
// ============================================

// ============================================
// 🌐 صفحه ورود (Login Page) با انتخاب اکانت
// ============================================

async function handleLoginPage(request, env) {
  // دریافت لیست اکانت‌ها از KV
  let accounts = [];
  try {
    const accountsData = await env.KV_BINDING.get('accounts');
    if (accountsData) {
      accounts = JSON.parse(accountsData);
    }
  } catch (e) {}
  
  // اگر اکانتی نبود، پیش‌فرض رو اضافه کن
  if (accounts.length === 0) {
    accounts = [{ phone: '+989378038736', apiId: '21052688', apiHash: 'dff2dac0409ffb456f3336ebb56f7d6c' }];
  }
  
  // ساخت گزینه‌های انتخاب شماره
  const phoneOptions = accounts.map(acc => 
    `<option value="${acc.phone}">📱 ${acc.phone}</option>`
  ).join('');
...
<body>
  <div class="container">
    ...
    <h1>ورود به سارا بات</h1>
    <p class="subtitle">💕 کد تایید رو وارد کن تا ربات فعال بشه</p>
    
    <div id="statusMsg" class="status-msg"></div>
    
    <form id="loginForm">
      <div class="input-group">
        <label>📱 انتخاب شماره</label>
      ...
      <div class="input-group">
        ...
        <input type="text" id="codeInput" placeholder="مثلاً ۱۲۳۴۵" maxlength="5" autocomplete="off" inputmode="numeric" pattern="[0-9]*" required>
      </div>
      
      <button type="submit" class="btn" id="submitBtn">✅ ارسال کد</button>
    </form>
    
    <div id="loading" class="loading">
      <div class="spinner"></div>
      <p style="margin-top:10px;color:#6c5ce7;">در حال ثبت کد...</p>
    </div>
    
    <div class="add-section">
      <button type="button" class="toggle-add" id="toggleAdd">➕ افزودن اکانت جدید</button>
      <div class="add-fields" id="addFields">
        <input type="text" id="newPhone" placeholder="شماره جدید (مثال: +989123456789)">
        <input type="text" id="newApiId" placeholder="API_ID">
        <input type="text" id="newApiHash" placeholder="API_HASH">
        <button type="button" class="btn btn-add" id="addAccountBtn">✅ افزودن اکانت</button>
        <div id="addResult" style="margin-top:10px;font-size:0.9em;"></div>
      </div>
    </div>
    
    <div class="info">
      🔑 کد به تلگرام شما ارسال شده
    </div>
  </div>
  ...
  <script>
    ...
    const newApiId = document.getElementById('newApiId');
    const newApiHash = document.getElementById('newApiHash');
    ...
    form.addEventListener('submit', async function(e) {
      ...
      if (!code || code.length < 5) {
        showStatus('❌ لطفاً کد ۵ رقمی رو کامل وارد کن!', 'error');
        return;
      }
      ...
      try {
        ...
        if (data.success) {
          showStatus('✅ ' + data.message + ' 🎉 ربات در حال فعال شدن...', 'success');
          setTimeout(() => {
            window.location.href = '/';
          }, 3000);
        } else {
          showStatus('❌ خطا: ' + (data.error || data.message || 'مشخص نیست!'), 'error');
        }
      } catch (error) {
        showStatus('❌ خطا در ارتباط با سرور! دوباره تلاش کن.', 'error');
      } finally {
    ...
    // نمایش/مخفی کردن فرم افزودن
    toggleAdd.addEventListener('click', function() {
      addFields.classList.toggle('show');
      this.textContent = addFields.classList.contains('show') ? '❌ بستن' : '➕ افزودن اکانت جدید';
    });

    // افزودن اکانت جدید
    addAccountBtn.addEventListener('click', async function() {
      const phone = newPhone.value.trim();
      const apiId = newApiId.value.trim();
      const apiHash = newApiHash.value.trim();
      
      if (!phone || phone.length < 10) {
        addResult.innerHTML = '❌ شماره رو درست وارد کن!';
        addResult.style.color = 'red';
        return;
      }
      if (!apiId || !apiHash) {
        addResult.innerHTML = '❌ API_ID و API_HASH رو وارد کن!';
        addResult.style.color = 'red';
        return;
      }
      
      addAccountBtn.disabled = true;
      addAccountBtn.textContent = '⏳ در حال افزودن...';
      ...
      try {
        ...
        if (data.success) {
          ...
          toggleAdd.textContent = '➕ افزودن اکانت جدید';
          setTimeout(() => {
            addResult.innerHTML = '';
          }, 3000);
        } else {
          addResult.innerHTML = '❌ ' + (data.error || 'خطا!');
          addResult.style.color = 'red';
        }
      } catch (error) {
        addResult.innerHTML = '❌ خطا در ارتباط با سرور!';
        addResult.style.color = 'red';
      } finally {
        addAccountBtn.disabled = false;
        addAccountBtn.textContent = '✅ افزودن اکانت';
      }
    });
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    h1 { font-size: 2.5em; color: #6c5ce7; margin-bottom: 10px; }
...
<body>
  <div class="container">
    <h1>🌸 سارا</h1>
    <p class="subtitle">💕 سلف‌بات مولتی اکانت</p>
  ...
  <script>
    fetch('/api/accounts')
      .then(r => r.json())
      .then(data => {
        document.getElementById('accountCount').textContent = data.accounts ? data.accounts.length : 0;
      })
      .catch(() => {});
  </script>
</body>
    ...
    if (path === '/submitcode' && request.method === 'POST') {
      try {
        ...
        const phone = body.phone || '+989378038736';
    ...
    if (path === '/api/add-account' && request.method === 'POST') {
      try {
        ...
        let accounts = [];
        try {
          const data = await env.KV_BINDING.get('accounts');
          if (data) accounts = JSON.parse(data);
        } catch (e) {}
        
        if (!accounts.find(a => a.phone === phone)) {
          accounts.push({ phone, apiId, apiHash });
          await env.KV_BINDING.put('accounts', JSON.stringify(accounts));
        }
        
        return new Response(JSON.stringify({ 
          success: true, 
          message: `اکانت ${phone} اضافه شد!` 
        }), {
          headers: { 'Content-Type': 'application/json' }
        });
      } catch (error) {
    ...
    return new Response(JSON.stringify({ 
      error: 'مسیر یافت نشد!',
      available: ['/', '/login', '/submitcode', '/api/accounts', '/api/add-account']
    }), {
      status: 404,
      headers: { 'Content-Type': 'application/json' }
    });
