...
// ============================================
// 📸 سارا - فقط عکس و فیلم (بدون خطا)
// ============================================

const BOT_TOKEN = 'توکن_ربات_تت'; // ← اینجا توکنتو بزار!
...
function getReply(text) {
  const lower = text.toLowerCase();
  if (lower.includes('سلام') || lower.includes('hello')) return '🌸 سلام عزیزم! حالت چطوره؟';
  if (lower.includes('چطوری') || lower.includes('حالت')) return '😍 خوبم عزیزم! تو چطوری؟';
  if (lower.includes('دوستت دارم') || lower.includes('عاشق')) return '💋 منم عاشقتم! خیلی زیاد!';
  if (lower.includes('بوس') || lower.includes('kiss')) return '💋 بیا ببوسمت!';
  if (lower.includes('بغل') || lower.includes('hug')) return '🤗 بیا بغلم!';
  if (lower.includes('دلم تنگه') || lower.includes('دلتنگ')) return '💕 دلمم برات تنگ شده!';
  if (lower.includes('ناراحت') || lower.includes('غمگین')) return '🤗 چرا ناراحتی عزیزم؟ بگو ببینم!';
  if (lower.includes('شب بخیر') || lower.includes('خواب')) return '🌙 شب بخیر عزیزم! رویاهای قشنگ ببین!';
  if (lower.includes('خداحافظ') || lower.includes('bye')) return '💕 خداحافظ عزیزم! منتظرت هستم!';
  return ['🌸 راستی چه خبر از خودت؟', '💕 چه حس قشنگی!', '😏 بیا بیشتر حرف بزنیم!'][Math.floor(Math.random() * 3)];
}

function detectCommand(text) {
  const lower = text.toLowerCase();
  if (lower.includes('ویدیو') || lower.includes('فیلم')) return 'video';
  if (lower.includes('عکس') || lower.includes('ببینم') || lower.includes('نشون بده')) return 'photo';
  return null;
}
...
async function sendPhoto(chatId) {
  const random = SARA_PHOTOS[Math.floor(Math.random() * SARA_PHOTOS.length)];
...
async function sendVideo(chatId) {
  const random = SARA_VIDEOS[Math.floor(Math.random() * SARA_VIDEOS.length)];
  await fetch(`https://api.telegram.org/bot${BOT_TOKEN}/sendVideo`, {
    ...
    body: JSON.stringify({
      chat_id: chatId,
      video: random,
      caption: '🎬 اینم فیلم سارا! 😍',
      supports_streaming: true,
      reply_markup: {
        inline_keyboard: [
          [{ text: '🎬 فیلم دیگه', callback_data: 'video' }],
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/webhook') {
      try {
        ...
        if (callback) {
          ...
          if (callback.data === 'photo') await sendPhoto(chatId);
          else if (callback.data === 'video') await sendVideo(chatId);
        ...
        if (message && message.text) {
          ...
          if (text === '/help') {
            await fetch(`https://api.telegram.org/bot${BOT_TOKEN}/sendMessage`, {
              ...
              body: JSON.stringify({
                chat_id: chatId,
                text: `🌸 **راهنمای سارا**\n\n📸 عکستو ببینم - عکس سارا\n🎬 فیلم بفرست - فیلم سارا\n\n💬 هر چیزی بگی جواب میده!`
              })
            });
            return new Response('OK');
          }
    ...
    return new Response('🌸 سارا - عکس و فیلم');
  }
};
