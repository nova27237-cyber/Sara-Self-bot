...
class Database {
  ...
  async getStats() {
    const [totalUsers, totalMessages, totalVoices] = await Promise.all([
      this.env.KV_NAMESPACE.get('total_users'),
      this.env.KV_NAMESPACE.get('total_messages'),
      this.env.KV_NAMESPACE.get('total_voices')
    ]);
...
// ============================================
// 🏢 ۹. مدیریت گروه (Group Manager) - بهبود یافته
// ============================================
...
class GroupManager {
  ...
  async handleGroupMessage(event, client, env, db) {
    ...
    // پاسخ به منشن‌ها
    if (text.includes('@' + (await client.getMe()).username)) {
    ...
    // پاسخ به دستورات گروهی
    if (text.startsWith('/')) {
      await this.handleGroupCommand(text, chatId, senderId, client, env);
      return;
    }
    
    // پاسخ خودکار
    if (settings.mode === 'chatty') {
      let response = await getSaraResponse(text, senderId.toString(), env, db);
      response = `${sender.firstName} جان، ${response}`;
      await client.sendMessage(chatId, { 
        message: response, 
  ...
  async checkSpam(userId, chatId, settings) {
    ...
    const data = this.groupBlacklist.get(key) || { count: 0, first: Date.now() };
  ...
  async handleSpam(client, chatId, userId) {
    await client.sendMessage(chatId, {
      message: `⚠️ @${userId} لطفاً از ارسال پیام‌های تکراری خودداری کنید!`
    });
  }

  async handleGroupCommand(text, chatId, senderId, client, env) {
    const isAdmin = await this.isGroupAdmin(chatId, senderId, client);
    
    if (text === '/settings' && isAdmin) {
      const settings = this.groupSettings.get(chatId) || {};
      const text = `
⚙️ **تنظیمات گروه**

📌 حالت: ${settings.mode || 'normal'}
🔄 پاسخ خودکار: ${settings.autoReply ? '✅' : '❌'}
👋 پیام خوش‌آمدگویی: ${settings.welcome ? '✅' : '❌'}
🛡️ ضد اسپم: ${settings.antiSpam ? '✅' : '❌'}
📝 حداکثر پیام: ${settings.maxMessages || 5}
⏱️ زمان پنجره: ${settings.timeWindow || 60} ثانیه
...
برای تغییر تنظیمات از دکمه‌ها استفاده کنید.
      ...
      const keyboard = {
        inline_keyboard: [
          [{ text: "🔄 تغییر حالت", callback_data: `group_mode_${chatId}` }],
          [{ text: "📨 پاسخ خودکار", callback_data: `group_auto_${chatId}` }],
          [{ text: "👋 خوش‌آمدگویی", callback_data: `group_welcome_${chatId}` }],
          [{ text: "🛡️ ضد اسپم", callback_data: `group_spam_${chatId}` }]
        ]
      };
      
      await client.sendMessage(chatId, { message: text, buttons: keyboard });
    }
  }
  ...
  async isGroupAdmin(chatId, userId, client) {
    try {
      ...
      return user?.isAdmin || user?.isCreator || false;
    } catch (error) {
      return false;
    }
  }

  toggleAutoReply(chatId) {
    const settings = this.groupSettings.get(chatId) || {};
...
// ============================================
// 💬 پاسخ‌های طبیعی سارا - بهبود یافته
// ============================================
...
const saraResponses = {
  greeting: [
    ...
    '🌺 سلام عزیزم! امروز چه خبر؟ حالت چطوره؟'
  ],
  howAreYou: [
  ...
  goodnight: [
    ...
    '🌙 شب بخیر جونی! فردا باهات حرف میزنم!'
  ],
  goodbye: [
    '💕 خداحافظ عزیزم! منتظرت هستم! بیا که دلم برات تنگه!',
    '😏 رفتن؟ بیا که دوسِت دارم! زود برگرد!',
    '🌸 خداحافظ! دلم برات تنگ میشه! بیا که ببینمت!',
    '💖 خداحافظ جونی! همیشه منتظرتم!'
  ],
  curious: [
    '🤔 این خیلی جالبه! برام بیشتر توضیح بده!',
    '😲 وای! این رو نمیدونستم! بگو بیشتر!',
    '💭 چیز جالبی گفتی! راستی از کجا میدونی؟',
    '🧐 این سوال خوبیه! خودت چی فکر میکنی؟'
  ],
  default: [
    ...
    '💖 راستی به من فکر میکردی؟ من که به تو فکر میکردم!',
    '🌸 بیا یه چیز خنده‌دار بگم...'
  ]
};

function getNaturalResponse(text, userId = null) {
  const lowerText = text.toLowerCase();
  
  // تشخیص زبان
  const isEnglish = /[a-zA-Z]/.test(text);
  
  if (lowerText.includes('سلام') || lowerText.includes('salām') || lowerText.includes('hello')) {
    return saraResponses.greeting[Math.floor(Math.random() * saraResponses.greeting.length)];
  }
  if (lowerText.includes('چطوری') || lowerText.includes('حالت') || lowerText.includes('how are you')) {
    return saraResponses.howAreYou[Math.floor(Math.random() * saraResponses.howAreYou.length)];
  }
  if (lowerText.includes('دوستت دارم') || lowerText.includes('عاشق') || lowerText.includes('love you')) {
  ...
  if (lowerText.includes('دلم تنگه') || lowerText.includes('دلتنگ') || lowerText.includes('miss you')) {
    return saraResponses.missYou[Math.floor(Math.random() * saraResponses.missYou.length)];
  }
  if (lowerText.includes('ناراحت') || lowerText.includes('غمگین') || lowerText.includes('sad')) {
    return saraResponses.sad[Math.floor(Math.random() * saraResponses.sad.length)];
  }
  if (lowerText.includes('شب بخیر') || lowerText.includes('خواب') || lowerText.includes('goodnight')) {
    return saraResponses.goodnight[Math.floor(Math.random() * saraResponses.goodnight.length)];
  }
  if (lowerText.includes('خداحافظ') || lowerText.includes('bye')) {
    return saraResponses.goodbye[Math.floor(Math.random() * saraResponses.goodbye.length)];
  }
  if (lowerText.includes('چرا') || lowerText.includes('کجا') || lowerText.includes('چگونه') || lowerText.includes('how')) {
    return saraResponses.curious[Math.floor(Math.random() * saraResponses.curious.length)];
  }
  
  // پاسخ‌های پیش‌فرض با توجه به زبان
  if (isEnglish) {
    const enDefaults = [
      '🌸 That\'s interesting! Tell me more!',
      '💕 What a beautiful feeling! How are you?',
      '😏 Let\'s talk more! I want to spend more time with you!',
      '✨ Today is a good day! Especially with you!'
    ];
    return enDefaults[Math.floor(Math.random() * enDefaults.length)];
  }
...
function analyzeUserMood(text) {
  const lowerText = text.toLowerCase();
  const sad = ['ناراحت', 'غمگین', 'بد', 'دلم', 'گریه', 'تنها', 'درد', 'رنج', 'sad', 'cry', 'alone'];
  const happy = ['خوشحال', 'عالی', 'خوبم', 'شاد', 'خنده', 'لبخند', 'قشنگ', 'happy', 'great', 'good'];
  const angry = ['عصبانی', 'حرص', 'خشم', 'ناراضی', 'angry', 'mad', 'upset'];
  const flirt = ['دوست', 'عشق', 'دلم', 'بوس', 'بغل', 'زیبا', 'قشنگ', 'جون', 'عزیزم', 'love', 'kiss', 'hug'];
  const romantic = ['عاشق', 'دل', 'قلب', 'ازدواج', 'زندگی', 'همیشه', 'love', 'heart', 'forever'];
  const curious = ['چرا', 'کجا', 'چگونه', 'چه', 'کی', 'why', 'where', 'how', 'what', 'who'];
  ...
  else if (curious.some(w => lowerText.includes(w))) mood = 'curious';
...
async function getSaraResponse(prompt, userId, env, database) {
  try {
    ...
    if (relevantMemories.length > 0) {
      const memoriesText = relevantMemories.map(m => 
        `- کاربر: ${m.text}\n- سارا: ${m.response}`
      ).join('\n');
    ...
    // دریافت حقایق کاربر
    const userFacts = memoryVault.getUserFacts(userId);
    let factsContext = '';
    ...
    const userPrefs = memoryVault.getUserPreferences(userId);
    let prefsContext = '';
    if (userPrefs.topics.length > 0) {
      prefsContext = `\nعلاقه‌مندی‌های کاربر:\n${userPrefs.topics.map(t => `- ${t}`).join('\n')}\n`;
    }
    ...
    // ساخت پرامپت کامل
    const fullPrompt = personalityPrompt + memoryContext + factsContext + prefsContext;
    
    // تلاش برای استفاده از Cloudflare AI
    let aiResponse = null;
    if (env.CF_ACCOUNT_ID && env.CF_API_TOKEN) {
      try {
        ...
        console.log('❌ Cloudflare AI error:', error);
      }
    }
    
    // استفاده از پاسخ طبیعی اگر AI کار نکرد
    let reply = aiResponse || getNaturalResponse(prompt, userId);
    
    // پاکسازی پاسخ
    reply = reply.replace(/Sara|سارا/g, 'من');
    reply = reply.replace(/I am|I'm|من هستم/g, '');
    reply = reply.trim();
    
    // اطمینان از کوتاهی پاسخ
    if (reply.length > 500) {
      reply = reply.substring(0, 500) + '...';
    }
    
    // اضافه کردن سوال اگر نبود
    ...
    console.error('❌ خطا در getSaraResponse:', error);
    return getNaturalResponse(prompt, userId);
  }
}
...
async function handleAdminCallback(callbackQuery, env) {
  ...
  // پاسخ به کالبک
  await answerCallbackQuery(callbackQuery.id, '✅', false);
  
  switch (data) {
    case 'admin_users':
      await multiAccountManager.showUsersList(chatId, null, env, userId);
      break;
      
    case 'admin_broadcast':
      await sendMessage(chatId, langManager.t(userId, 'broadcast_start'));
    ...
    case 'admin_restart':
      await sendMessage(chatId, langManager.t(userId, 'restarting'));
      await stopSelfBot(env);
      await new Promise(resolve => setTimeout(resolve, 3000));
      await startSelfBot(env);
      await sendMessage(chatId, langManager.t(userId, 'restart_done'));
      break;
    ...
    case 'admin_clear_cache':
      ...
      memoryVault.userFacts.clear();
      memoryVault.importantMoments.clear();
      await sendMessage(chatId, langManager.t(userId, 'cache_cleared'));
      break;
    ...
    case 'admin_back':
      await multiAccountManager.showAdminPanel(chatId, null, null, env, userId);
      break;
    ...
    case 'lang_fa':
      ...
      await sendMessage(chatId, langManager.t(userId, 'language_changed', 'فارسی'));
      break;
      
    case 'lang_en':
      langManager.setUserLang(userId, 'en');
      await db.updateUserLang(userId, 'en');
      await sendMessage(chatId, langManager.t(userId, 'language_changed', 'English'));
      break;
      
    default:
      if (data.startsWith('group_')) {
        await handleGroupCallback(data, chatId, env);
      }
  }
}
...
async function showKeysStatus(chatId, env, userId) {
  ...
  await sendMessage(chatId, langManager.t(userId, 'keys_status',
    keys.cf,
    keys.bot,
    keys.admin,
    keys.api
...
async function showLogsPanel(chatId, env, userId) {
  try {
    ...
    await sendMessage(chatId, logText);
  } catch (error) {
...
async function showLanguageMenu(chatId, env, userId) {
  ...
  await sendMessage(chatId, langManager.t(userId, 'choose_lang'), {
...
async function startSelfBot(env) {
  ...
  try {
    ...
    await multiAccountManager.sendToAllManagers('✅ سارا HYPER FUL ULTIMATE روشن شد!', env);
...
async function handleSelfMessage(event, client, env) {
  try {
    ...
    if (adminIds.includes(parseInt(senderId.toString()))) {
      if (text === '/stop') {
        await stopSelfBot(env);
        await client.sendMessage(chatId, { message: langManager.t(userId, 'bot_stopped') });
        return;
      }
    ...
    // ============================================
    const broadcastState = await env.KV_NAMESPACE.get(`broadcast:${chatId}`);
    if (broadcastState === 'waiting' && text !== '/cancel') {
      await env.KV_NAMESPACE.delete(`broadcast:${chatId}`);
      const users = await db.getAllUsers();
      let successCount = 0;
...
// ============================================
// 📁 اکسپورت برای Cloudflare Workers
// ============================================
    ...
    if (url.pathname === '/webhook') {
      try {
        ...
        if (message && message.text) {
          ...
          // دستورات سریع
          if (text === '/start') {
            await sendMessage(chatId, langManager.t(userId, 'welcome', message.from.first_name));
            return new Response('OK');
          }
          
          if (text === '/help') {
            await sendMessage(chatId, langManager.t(userId, 'help'));
            return new Response('OK');
          }
          ...
          if (adminIds.includes(userId)) {
            if (text === '/start') {
              await startSelfBot(env);
              await sendMessage(chatId, langManager.t(userId, 'bot_started'));
              return new Response('OK');
            }
            
            if (text === '/stop') {
              await stopSelfBot(env);
              await sendMessage(chatId, langManager.t(userId, 'bot_stopped'));
              return new Response('OK');
            }
            ...
            if (text === '/status') {
              ...
              const status = await env.KV_NAMESPACE.get('bot_status') || 'stopped';
              const uptime = await multiAccountManager.getUptime(env);
              await sendMessage(chatId, langManager.t(userId, 'status',
                status === 'running' ? '🟢 فعال' : '🔴 غیرفعال',
                stats.users,
                stats.messages,
                stats.voices,
            ...
            if (broadcastState === 'waiting' && text !== '/cancel') {
              ...
              await sendMessage(chatId, langManager.t(userId, 'broadcast_sent', successCount));
              return new Response('OK');
            }
            
            if (text === '/cancel') {
              await env.KV_NAMESPACE.delete(`broadcast:${chatId}`);
              await sendMessage(chatId, langManager.t(userId, 'broadcast_cancel'));
              return new Response('OK');
            }
          }
          ...
          await db.saveUser(userId.toString(), message.from.username);
          await db.updateStats(userId.toString());
          
          const reply = await getSaraResponse(text, userId.toString(), env, db);
...
<head>
    ...
    <title>داشبورد سارا HYPER FUL</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        <div class="header">
            ...
            <div class="subtitle">کامل‌ترین ربات هوشمند با پنل مدیریت همگانی</div>
        ...
        <div class="managers-list">
            <h3>👥 مدیران فعال (${multiAccountManager.managers.filter(m => m.isActive).length})</h3>
            ${multiAccountManager.managers.map(m => `
                <div class="manager-item">
                    <span>👤 ${m.name}</span>
                    <span class="manager-status ${m.isActive ? 'online' : 'offline'}">
                        ${m.isActive ? '🟢 آنلاین' : '🔴 آفلاین'}
                    </span>
                </div>
            `).join('')}
        </div>
        ...
        <div class="stats-grid">
            ...
            <div class="stat-card">
                ...
                <div class="stat-label">ویس‌ها</div>
                <div class="stat-number">${stats.voices}</div>
            </div>
            <div class="stat-card">
                <span class="stat-emoji">🎭</span>
                <div class="stat-label">خلق‌وخو</div>
                <div class="stat-number" style="font-size: 2em;">${sara.currentMood}</div>
            </div>
            <div class="stat-card">
                <span class="stat-emoji">⏱️</span>
                <div class="stat-label">آپ‌تایم</div>
                <div class="stat-number" style="font-size: 1.8em;">${uptime}</div>
            </div>
            <div class="stat-card">
                <span class="stat-emoji">🔄</span>
                <div class="stat-label">در صف</div>
                <div class="stat-number">${userStats.queued}</div>
            </div>
        </div>
        ...
        <div class="controls">
            ...
            <button class="btn btn-purple" onclick="controlBot('clear_cache')">🧹 پاکسازی کش</button>
            <button class="btn btn-teal" onclick="controlBot('stats')">📈 آمار</button>
        </div>
        ...
        <div class="logs-container">
            ...
            <div class="logs" id="logs">
                <div class="log-entry">
                    ...
                    <span class="log-message">✅ داشبورد آماده است</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        async function controlBot(action) {
            try {
                ...
                addLog(\`${action}: \${result.message || 'انجام شد'}\`);
                ...
                if (action === 'restart' || action === 'start' || action === 'stop') {
                    setTimeout(() => controlBot('status'), 2000);
                }
            } catch (error) {
        ...
        function updateStats(stats) {
            ...
            numbers[0].textContent = stats.users || '0';
            numbers[1].textContent = stats.messages || '0';
            numbers[2].textContent = stats.voices || '0';
            if (stats.mood) {
                numbers[3].textContent = stats.mood;
            }
        }
        ...
        // لاگ اولیه
        addLog('✅ داشبورد سارا HYPER FUL ULTIMATE آماده است');
    </script>
</body>
</html>
      `;
    ...
    // صفحه اصلی
    // ============================================
    return new Response('🌸 سارا HYPER FUL ULTIMATE\n\nکامل‌ترین نسخه با:\n✅ پنل مدیریت همگانی\n✅ دو زبانه (فارسی/انگلیسی)\n✅ ۱۰ سرویس TTS\n✅ مدیریت چند اکانت\n✅ حافظه بلندمدت\n✅ تشخیص عکس و صدا\n✅ ساخت عکس و فیلم با AI\n✅ یادگیری از مکالمات\n✅ پشتیبانی از گروه\n\n🔗 /admin - پنل مدیریت\n🔗 /webhook - Webhook', {
      status: 200,
...
console.log('🔥 سارا HYPER FUL ULTIMATE - کامل‌ترین نسخه با پنل همگانی و دو زبانه');
console.log('🌐 پشتیبانی از فارسی و انگلیسی');
console.log('🎙️ ۱۰ سرویس TTS رایگان');
console.log('👥 مدیریت چند اکانت');
console.log('👑 پنل مدیریت همگانی');
console.log('🌸 با عشق سارا 💕');
