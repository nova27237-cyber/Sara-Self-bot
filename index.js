...
// ✅ توکن ثابت
const BOT_TOKEN = '8568499654:AAFyLZs7Ex9XzIy3wCUb9l9_gYQ2B_rF-Ss';
...
async function sendMessage(chatId, text, options = {}) {
  try {
    ...
    if (!response.ok) {
      console.error('❌ ارسال ناموفق:', await response.text());
    }
...
export default {
  async fetch(request, env) {
    ...
    // ============================================
    // Webhook Handler - نسخه ساده
    // ============================================
    if (url.pathname === '/webhook') {
      try {
        console.log('📨 Webhook دریافت شد');
        ...
        // ============================================
        // پاسخ‌های ساده (بدون وابستگی)
        // ============================================
        let reply = '';
        
        if (text === '/start') {
          reply = `🌸 سلام ${firstName}! به سارا خوش آمدی! 💕\n\nمن سارا هستم، ۲۲ ساله! چطور میتونم کمکت کنم؟ 😊`;
        } else if (text === '/help') {
          reply = `📖 **راهنمای سارا** 🌸\n\n/start - شروع\n/help - راهنما\n/status - وضعیت\nهر چیزی بپرسی جواب میدم! 💕`;
        } else if (text === '/status') {
          reply = `✅ سارا آنلاین هستم!\n⏱️ ${new Date().toLocaleTimeString('fa-IR')}\n🌸 منتظر حرفات هستم!`;
        } else if (text.includes('سلام') || text.includes('hi')) {
          const greetings = [
            `🌸 سلام ${firstName} عزیزم! حالت چطوره؟ دلم برات تنگ شده بود! 💕`,
            `😏 سلام جونی! چقد دیر کردی! بیا پیشم که دوسِت دارم!`,
            `💋 سلام! راستی چطوری؟ من که امروز خیلی خوبم!`
          ];
          reply = greetings[Math.floor(Math.random() * greetings.length)];
        } else if (text.includes('دوستت دارم') || text.includes('عاشق')) {
          reply = `💕 منم عاشقتم ${firstName} جان! خیلی زیاد! 😍`;
        } else if (text.includes('ناراحت') || text.includes('غمگین')) {
          reply = `🤗 چرا ناراحتی ${firstName} جان؟ بگو ببینم چی شده؟ من اینجام که آرومت کنم! 💕`;
        } else {
          const defaults = [
            `🌸 چه حرف قشنگی ${firstName} جان! راستی امروز چیکار کردی؟`,
            `💕 ${firstName} عزیزم، خوشحالم که باهات حرف میزنم!`,
            `😏 ${firstName} جان، بیا بیشتر حرف بزنیم! دلم میخواد بیشتر باهات باشم!`
          ];
        ...
        return new Response('OK', { status: 200 }); // ← همیشه ۲۰۰ برگردون!
      }
    }
