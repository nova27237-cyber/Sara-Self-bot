...
// ============================================
// 📁 index.js - سارا ربات توکنی برای Cloudflare Workers
// ============================================

// ============================================
// ⚙️ تنظیمات (فقط یک بار)
// ============================================
...
async function deleteMessage(chatId, messageId) {
  try {
    await fetch(
      `https://api.telegram.org/bot${CONFIG.BOT_TOKEN}/deleteMessage`,
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
...
async function sendTyping(chatId) {
  try {
    await fetch(
      `https://api.telegram.org/bot${CONFIG.BOT_TOKEN}/sendChatAction`,
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
    ...
    console.error('❌ خطا در تایپینگ:', error);
  }
}

// ============================================
// 🤖 پاسخ‌های هوشمند سارا (فقط یک بار)
// ============================================

function getSaraResponse(text) {
  const lower = text.toLowerCase();
  ...
  if (lower.includes('دوستت دارم') || lower.includes('عاشق') || lower.includes('عشق')) {
    const replies = [
      '💋 منم عاشقتم! خیلی زیاد!',
      '😏 میدونم! منم دوسِت دارم!',
      '💕 عاشقتم! بیا که ببینمت!',
...
// ============================================
// 📋 دیکشنری پاسخ‌های سریع
// ============================================

const QUICK_REPLIES = {
  'سلام': '🌸 سلام عزیزم!',
  'خوبی': '😍 خوبم! تو چطوری؟',
  'چطوری': '💕 عالی! مخصوصاً وقتی تو هستی!',
  'دوستت دارم': '💋 منم عاشقتم!',
  'عاشقتم': '💕 منم عاشقتم!',
  'ناراحتم': '🤗 چرا ناراحتی عزیزم؟ بگو ببینم!',
  'خداحافظ': '💕 خداحافظ عزیزم!',
  'بای': '🌸 خداحافظ! دلم برات تنگ میشه!',
  'help': '📖 برای راهنما /help را بفرستید',
  'راهنما': '📖 برای راهنما /help را بفرستید'
};

// ============================================
// 🔍 تابع دریافت اطلاعات ربات
// ============================================
    ...
    if (path === '/webhook') {
      try {
        ...
        if (text === '/about') {
          await sendMessage(chatId,
            `🌸 **درباره سارا**\n\n` +
            `سارا یه ربات دوست داشتنی و مهربونه!\n` +
            `💕 هدفش شاد کردن شماست.\n\n` +
        ...
        // ===== /delete =====
        if (text === '/delete' && msg.reply_to_message) {
          try {
            await deleteMessage(chatId, msg.reply_to_message.message_id);
            await sendMessage(chatId, '🗑️ پیام حذف شد!');
          } catch (error) {
            await sendMessage(chatId, '❌ خطا در حذف پیام!');
          }
          return new Response('OK');
        }
        ...
        if (!isGroup) {
          ...
          if (QUICK_REPLIES[lowerText]) {
            await sendMessage(chatId, QUICK_REPLIES[lowerText]);
            return new Response('OK');
          }

          // ===== پاسخ هوشمند =====
          const response = getSaraResponse(text);
          await sendMessage(chatId, response);
        } else {
          // ===== در گروه‌ها فقط با منشن پاسخ بده =====
          const me = await getMe();
          if (me && text.includes(`@${me.username}`)) {
            await sendTyping(chatId);
            await new Promise(r => setTimeout(r, 500 + Math.random() * 1500));
            
            let cleanText = text.replace(new RegExp(`@${me.username}`, 'g'), '').trim();
            if (!cleanText) cleanText = 'سلام';
            
            const response = getSaraResponse(cleanText);
            await sendMessage(chatId, `👤 ${msg.from?.first_name || 'کاربر'} جان، ${response}`, {
              replyTo: messageId
            });
          }
        }
