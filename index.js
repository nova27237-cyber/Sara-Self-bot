...
class LearningEngine {
  constructor() {
    this.patterns = new Map();          // الگوهای یادگرفته شده
    this.responses = new Map();         // پاسخ‌های ذخیره شده
    this.userPreferences = new Map();   // ترجیحات کاربران
    this.conversationFlows = new Map(); // جریان مکالمات
    this.learnedPhrases = new Map();    // عبارات یادگرفته شده
    this.userPatterns = new Map();      // الگوهای هر کاربر
  }
}
...
class SaraPersonality {
  constructor() {
    ...
    this.traits = {
      humor: 0.9,        // شوخ‌طبعی
      flirt: 0.95,       // معاشقه/لطافت
      empathy: 0.98,     // همدلی
      playfulness: 0.85, // بازیگوشی
      romance: 1.0,      // عاشقانه
      patience: 0.8,     // صبر
      creativity: 0.9,   // خلاقیت
      shyness: 0.3,      // خجالتی بودن
      energy: 0.9,       // انرژی
      kindness: 0.95,    // مهربانی
      curiosity: 0.9,    // کنجکاوی
      optimism: 0.85     // خوش‌بینی
    };
  }
}
    };
...
class MemoryVault {
  constructor() {
    this.memories = new Map();           // خاطرات عمومی
    this.userFacts = new Map();          // حقایق ثابت کاربر
    this.importantMoments = new Map();   // لحظات خاص
    this.conversationContext = new Map(); // بافت مکالمه
    this.userPreferences = new Map();    // تنظیمات کاربر
    this.userMoods = new Map();          // حالات روحی
    this.userNames = new Map();          // اسامی
    this.userBirthdays = new Map();      // تاریخ تولدها
    this.userInterests = new Map();      // علایق
  }
}
...
// userManager حذف شد چون نیاز نبود
