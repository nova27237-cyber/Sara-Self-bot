// ================================================================
// 📁 index.js - سارا AI Ultimate (نسخه نهایی 12.0.0)
// ================================================================
// 
// 📌 خطوط: 20000+
// 📌 تاریخ: 2026
...
// ✅ ربات‌ساز حرفه‌ای مثل BotFather
// ✅ سیستم پرداخت و سکه
// ✅ ۲۸ شخصیت با دسته‌بندی
// ✅ هوش مصنوعی با DeepSeek و OpenAI
// ✅ محدودیت هوشمند (VIP / رایگان)
// ✅ پنل ادمین کامل
// ================================================================
  ...
  async saveUser(userId, userData = {}) {
    ...
    if (!existing) {
      const newUser = {
        ...
        experience: 0,
        achievements: [],
        settings: { language: 'fa', notifications: true, darkMode: false },
        stats: { totalMessages: 0, totalImages: 0, totalVoices: 0, totalCommands: 0, longestStreak: 0, currentStreak: 0, lastMessageDate: null },
        preferences: { favoriteTopics: [], blockedWords: [], customGreeting: null, character: 'sara' },
        metadata: { device: null, platform: null, botVersion: BOT_VERSION },
        notes: {},
        mood: 'neutral'
      };
...
// ================================================================
// 💎 سیستم محدودیت هوشمند
// ================================================================

class SmartLimitSystem {
  constructor(db) {
    this.db = db;
    this.FREE_LIMITS = { messages: 50, voices: 50, images: 50, bots: 1, personalities: 5 };
  ...
  async checkLimits(userId) {
    ...
    if (isPremium) {
      return { messages: Infinity, voices: Infinity, images: Infinity, bots: Infinity, personalities: Infinity };
    }
  ...
  async getUserUsage(userId) {
    ...
    if (!usage) {
      usage = { messages: 0, voices: 0, images: 0, bots: 0, resetDate: Date.now() };
    }
    const today = new Date().setHours(0, 0, 0, 0);
  ...
  async incrementUsage(userId, type) {
    const key = `usage_${userId}`;
    let usage = await this.db.getJSON(key) || { messages: 0, voices: 0, images: 0, bots: 0, resetDate: Date.now() };
  ...
  async canUse(userId, type) {
    ...
    return { allowed: false, message: this.getLimitReachedMessage(userId, type) };
  }

  getLimitReachedMessage(userId, type) {
    const emojis = { messages: '💬', voices: '🎤', images: '🎨', bots: '🤖' };
    const names = { messages: 'پیام', voices: 'ویس', images: 'تصویر', bots: 'ربات' };
    return `
...
class PersonalityManager {
  ...
  getPersonalitiesByCategory() {
    const categories = {};
    for (const p of Object.values(this.personalities)) {
  ...
  searchPersonality(query) {
    ...
    for (const [id, p] of Object.entries(this.personalities)) {
      if (p.name.includes(query) || p.desc.includes(query) || p.city.includes(query) ||
          p.job.includes(query) || p.traits.some(t => t.includes(query))) {
        results.push({ id, ...p });
      }
    }
    return results;
...
class SaraAI {
  ...
  getFallbackResponse(text, personality = null) {
    ...
    const keywords = {
      'سلام': ['سلام عزیزم! چه خبر؟ 🌸', 'سلام! خوش اومدی! حالت چطوره؟ 💕'],
      'خوب': ['خوبم عزیزم! ممنون که پرسیدی! تو چطوری؟ 🌸', 'عالی! با تو که هستم همیشه خوبم! 💕'],
      'دوستت دارم': ['منم عاشقتم! بیشتر از هر چیزی! 💕', 'دوستت دارم عزیزم! تو بهترینی! 🌸'],
      'دلم': ['دلم برات تنگ شده بود! بیا بغلم! 🤗', 'دل منم همیشه با توئه عزیزم! 💕']
    };
...
// ================================================================
// 📨 توابع کمکی (با پشتیبانی از فایل)
// ================================================================
...
class TelegramHelper {
  ...
  async getMe() {
    if (this._botInfo && (Date.now() - this._botInfoTime) < 60000) {
    ...
    try {
      ...
      const data = await response.json();
      if (data.ok) {
        this._botInfo = data.result;
        this._botInfoTime = Date.now();
        return data.result;
      }
    } catch (e) {}
    return null;
  ...
  async sendPhoto(chatId, photoData, caption = '', options = {}) {
    try {
      ...
      } else if (photoData instanceof ArrayBuffer || photoData instanceof Uint8Array) {
      ...
      if (options.keyboard) formData.append('reply_markup', JSON.stringify(options.keyboard));
  ...
  async sendVoice(chatId, audioBuffer, caption = '', options = {}) {
    try {
      const formData = new FormData();
      formData.append('chat_id', chatId.toString());
      ...
      const data = await response.json();
      if (!data.ok) console.error('❌ sendVoice error:', data.description);
      return data;
    } catch (error) {
...
class SaraKeyboard {
  constructor() {
    this.main = {
      inline_keyboard: [
        [{ text: "🤖 هوش مصنوعی", callback_data: "sara_ai" }],
        [{ text: "👤 پروفایل", callback_data: "sara_profile" }, { text: "📊 آمار", callback_data: "sara_stats" }],
        [{ text: "🎯 دستاوردها", callback_data: "sara_achievements" }, { text: "💎 VIP", callback_data: "sara_vip" }],
        [{ text: "🤖 ربات‌ساز", callback_data: "sara_botbuilder" }],
        [{ text: "🔙 بازگشت", callback_data: "back" }]
      ]
    };

    // کیبورد شخصیت‌ها با دسته‌بندی
    const categories = {
      'صمیمی': ['sara', 'elsana', 'diana', 'sanaz'],
      'هنری': ['mahtab', 'mona', 'tanaz', 'avin', 'ariana', 'baran', 'negar', 'billie', 'zara', 'ava'],
      'پرانرژی': ['hasti', 'rosha', 'jax'],
      'خلاق': ['setareh', 'romita', 'zara', 'ava'],
      'فنی': ['malika', 'elena', 'nova', 'luna', 'hamid'],
      'ماجراجو': ['darya'],
      'دانا': ['helia', 'aria'],
      'مرموز': ['lilith', 'cipher'],
      'ترسناک': ['leatherface'],
    ...
    for (const [cat, ids] of Object.entries(categories)) {
      ...
      for (const id of ids) {
        ...
        if (p) {
          row.push({ text: `${p.emoji} ${p.name}`, callback_data: `char_${id}` });
        }
      }
      if (row.length) keyboard.push(row);
    }
    keyboard.push([{ text: "🔙 بازگشت", callback_data: "sara_back" }]);
    this.characters = { inline_keyboard: keyboard };
    ...
    // کیبورد ربات‌ساز (مثل BotFather)
    this.botbuilder = {
      inline_keyboard: [
        [{ text: "➕ ساخت ربات جدید", callback_data: "bb_start" }],
...
// ================================================================
// 🤖 سیستم ربات‌ساز حرفه‌ای (مثل BotFather)
// ================================================================
...
class BotBuilder {
  ...
  // ===== شروع ساخت ربات =====
  async startBuild(userId) {
    const user = await this.db.getUser(userId);
    const isPremium = await this.db.isPremium(userId);
    const cost = isPremium ? 0 : 500;
    
    if (!isPremium && (user.coins || 0) < cost) {
      return {
        error: 'سکه کافی نیست',
        needed: cost,
        has: user.coins || 0,
    ...
    return {
      success: true,
      step: 'name',
      message: '🤖 **ساخت ربات جدید**\n\n' +
               'لطفاً **نام** ربات رو وارد کن:\n' +
               'مثال: `ربات من`\n\n' +
               (isPremium ? '🌟 **VIP** - هزینه صفر' : `💰 هزینه: ${cost} سکه (کسر شد)`)
    };
  }

  // ===== پردازش مرحله‌ای =====
  async processBuildStep(userId, text, callbackData = null) {
    const state = this.buildStates.get(userId);
    if (!state) return { error: 'هیچ فرآیندی فعال نیست', action: 'start' };

    const { step, data } = state;

    if (step === 'name') {
      if (!text || text.length < 2) {
        return { error: 'نام باید حداقل ۲ حرف باشه', step: 'name' };
      }
      data.name = text;
      state.step = 'personality';
      this.buildStates.set(userId, state);

      const personalities = this.personalityManager.getPersonalityList();
      let keyboard = { inline_keyboard: [] };
      for (const p of personalities.slice(0, 20)) {
        keyboard.inline_keyboard.push([{ text: `${p.emoji} ${p.name}`, callback_data: `build_pers_${p.id}` }]);
      }
      keyboard.inline_keyboard.push([{ text: "❌ انصراف", callback_data: "build_cancel" }]);

      return {
        step: 'personality',
        message: `✅ نام ربات: **${data.name}**\n\n` +
                 '🎭 حالا **شخصیت** ربات رو انتخاب کن:',
        keyboard: keyboard
      };
    }

    if (step === 'personality' && callbackData && callbackData.startsWith('build_pers_')) {
      const persId = callbackData.replace('build_pers_', '');
      const personality = this.personalityManager.getPersonality(persId);
      if (!personality) {
        return { error: 'شخصیت نامعتبر', step: 'personality' };
      }
      data.personality = persId;
      state.step = 'welcome';
      this.buildStates.set(userId, state);

      return {
        step: 'welcome',
        message: `✅ شخصیت: **${personality.emoji} ${personality.name}**\n\n` +
                 '📝 حالا **پیام خوش‌آمدگویی** رو وارد کن (یا /skip برای رد کردن):',
        keyboard: null
      };
    }
    ...
    if (step === 'welcome') {
      if (text && text !== '/skip') {
        ...
        data.welcome = `سلام! به ربات ${data.name} خوش اومدی! 🌸`;
      }
      state.step = 'token';
      this.buildStates.set(userId, state);
      ...
      return {
        step: 'token',
        message: `✅ پیام خوش‌آمدگویی تنظیم شد.\n\n` +
                 '🔑 حالا **توکن** ربات رو از @BotFather بگیر و وارد کن:\n' +
                 '(مثلاً `123456:ABC-DEF...`)',
        keyboard: null
      };
    }
    ...
    if (step === 'token') {
      if (!text || text.length < 10) {
        return { error: 'توکن نامعتبر (حداقل ۱۰ کاراکتر)', step: 'token' };
      }
      try {
        const test = await fetch(`https://api.telegram.org/bot${text}/getMe`);
        const result = await test.json();
        if (!result.ok) {
          return { error: '❌ توکن نامعتبر! دوباره وارد کن', step: 'token' };
        }
        data.token = text;
        data.botInfo = result.result;
      } catch (e) {
        return { error: '❌ خطا در تست توکن! دوباره وارد کن', step: 'token' };
      }
      ...
      return {
        step: 'confirm',
        message: `✅ توکن معتبر است! 🤖 @${data.botInfo.username}\n\n` +
                 '📋 **خلاصه ربات:**\n' +
                 `نام: ${data.name}\n` +
                 `شخصیت: ${this.personalityManager.getPersonality(data.personality).name}\n` +
                 `پیام خوش‌آمدگویی: ${data.welcome}\n` +
                 `توکن: \`${data.token}\`\n\n` +
                 '✅ برای **ساخت نهایی** دکمه زیر رو بزن:',
        keyboard: {
          inline_keyboard: [
            [{ text: "✅ ساخت ربات", callback_data: "build_final" }],
            [{ text: "❌ انصراف", callback_data: "build_cancel" }]
          ]
        }
      };
    }

    if (step === 'confirm' && callbackData === 'build_final') {
      const bot = await this.createBot(userId, {
        name: data.name,
      ...
      const activated = await this.activateBot(bot.id);
      this.buildStates.delete(userId);

      if (activated) {
        return {
          done: true,
          message: `🎉 **ربات ساخته و فعال شد!**\n\n` +
                   `🤖 @${data.botInfo.username}\n` +
                   `📛 نام: ${bot.name}\n` +
                   `🆔 آیدی: \`${bot.id}\`\n\n` +
                   `💡 با دستورات زیر مدیریتش کن:\n` +
                   `/bb list - لیست ربات‌ها\n` +
                   `/bb info ${bot.id} - اطلاعات\n` +
                   `/bb delete ${bot.id} - حذف`,
          keyboard: this.getManageKeyboard(bot.id)
        };
      } else {
        return {
          done: true,
          error: '❌ خطا در فعال‌سازی ربات! لطفاً بعداً تلاش کن.',
          message: '❌ خطا در فعال‌سازی'
        };
      }
    }

    if (callbackData === 'build_cancel') {
      this.buildStates.delete(userId);
      return {
        done: true,
        message: '❌ فرآیند ساخت ربات لغو شد.',
        keyboard: this.getMainKeyboard()
      };
    }

    return { error: 'مرحله نامعتبر', step: step };
  }

  // ===== کیبوردهای مدیریت =====
  getManageKeyboard(botId) {
    return {
      inline_keyboard: [
        [{ text: "📊 آمار", callback_data: `bb_stats_${botId}` }],
        [{ text: "🗑️ حذف", callback_data: `bb_delete_${botId}` }],
        [{ text: "🔙 بازگشت به لیست", callback_data: "bb_list" }]
      ]
    };
  }
  ...
  async getBot(botId) {
    ...
    if (bot) this.bots.set(botId, bot);
    return bot;
  }
  ...
  async updateBot(botId, updates) {
    ...
    await this.db.setJSON(`bot_${botId}`, bot);
    this.bots.set(botId, bot);
    if (this.activeBots.has(botId)) this.activeBots.set(botId, bot);
    return bot;
  }
  ...
  async deleteBot(botId) {
    ...
    if (!bot) return false;
    if (bot.token) {
      try { await fetch(`https://api.telegram.org/bot${bot.token}/deleteWebhook`); } catch (e) {}
    }
    ...
    try {
      ...
      for (const key of keys.keys) {
        ...
        if (botData) {
          ...
          if (bot.owner === userId) bots.push(bot);
        }
      }
    } catch (error) {}
    return bots;
  ...
  async generateBotResponse(bot, text, userId, history) {
    for (const cmd of bot.commands) {
      if (text.includes(cmd.keyword) || text.startsWith(cmd.keyword)) {
        return { response: cmd.response, type: 'command', command: cmd.keyword };
      }
    }
    if (bot.aiEnabled) {
      ...
      return { response: response, type: 'ai', personality: bot.personality };
    }
    return { response: bot.welcomeMessage || 'سلام! چطور میتونم کمک کنم؟ 🌸', type: 'welcome' };
  }
  ...
  async getBotStats(botId) {
    ...
    for (const uid of users) {
      ...
      if (user && (Date.now() - user.lastSeen) < 24 * 60 * 60 * 1000) activeCount++;
    }
    return {
...
class BotHandler {
  ...
  async handleMessage(msg) {
    try {
      ...
      // ===== چک کردن وضعیت ساخت ربات =====
      const buildState = this.botBuilder.buildStates.get(userId);
      if (buildState && !text.startsWith('/')) {
        const result = await this.botBuilder.processBuildStep(userId, text);
        if (result.done) {
          await this.helper.sendMessage(chatId, result.message, {
            replyTo: messageId,
            keyboard: result.keyboard || this.keyboard.getKeyboard('main')
          });
        } else if (result.error) {
          await this.helper.sendMessage(chatId, `❌ ${result.error}`, { replyTo: messageId });
        } else {
          await this.helper.sendMessage(chatId, result.message, {
            replyTo: messageId,
            keyboard: result.keyboard || undefined
          });
        }
        return;
      }
      ...
      await this.helper.sendMessage(msg.chat.id, '🌸 یه مشکلی پیش اومد! لطفاً دوباره تلاش کن.');
    }
  }
  ...
  async handlePrivateMessage(msg) {
    ...
    try {
      ...
      await this.helper.sendMessage(chatId, '🌸 یه مشکلی پیش اومد! لطفاً دوباره تلاش کن.', {
        replyTo: messageId,
        keyboard: this.keyboard.getKeyboard('main')
      });
    }
  ...
  async handleGroupMessage(msg) {
    ...
    try {
      ...
      console.error('❌ handleGroupMessage error:', error);
    }
  }
  ...
  async handleCommand(msg) {
    ...
    if (command === '/start') {
      ...
      await this.helper.sendMessage(chatId, 
        ...
        `📸 با /img تصویر بساز\n` +
        `🎤 با /voice ویس بساز\n` +
        `🔍 با /search عکس پیدا کن\n` +
        `🤖 با /botbuilder ربات شخصی بساز\n\n` +
        `💰 **کاملاً رایگان!**\n\n` +
        `🌸 برای پنل /sara بفرست`,
        { replyTo: messageId, keyboard: this.keyboard.getKeyboard('main') }
      );
    ...
    if (command === '/help') {
      await this.helper.sendMessage(chatId, 
        ...
        '/search [عبارت] - جستجوی تصویر\n' +
        '/voice [متن] - تبدیل متن به ویس\n' +
        '/new - پاک کردن حافظه\n' +
        '/profile - پروفایل\n' +
        '/stats - آمار من\n' +
    ...
    if (command === '/img') {
      ...
      try {
        ...
        if (response.ok) {
          ...
          await this.helper.sendPhoto(chatId, imageBuffer, `🎨 ${prompt}`, {
            replyTo: messageId,
            keyboard: this.keyboard.getKeyboard('main')
          });
    ...
    if (command === '/search') {
      ...
      try {
        ...
        for (const hit of data.hits.slice(0, 5)) {
          const imgResp = await fetch(hit.webformatURL);
          if (imgResp.ok) {
            const buffer = await imgResp.arrayBuffer();
            await this.helper.sendPhoto(chatId, buffer, `🔍 ${query}`, { replyTo: messageId });
          }
    ...
    if (command === '/voice') {
      ...
      try {
        ...
        if (response.ok) {
          ...
          await this.helper.sendVoice(chatId, audioBuffer, `🎤 ${textToSpeak.substring(0, 50)}...`, {
            replyTo: messageId,
            keyboard: this.keyboard.getKeyboard('main')
          });
          ...
          await this.helper.sendMessage(chatId, `🎤 ${textToSpeak}`, {
            replyTo: messageId,
            keyboard: this.keyboard.getKeyboard('main')
          });
        }
    ...
    if (command === '/characters') {
      const categories = this.personalityManager.getPersonalitiesByCategory();
    ...
    if (isAdmin) {
      ...
      if (command === '/ban') {
        ...
        if (user) {
          ...
          await this.helper.sendMessage(targetId, '🚫 شما توسط ادمین بن شدید!');
        } else {
          await this.helper.sendMessage(chatId, '❌ کاربر یافت نشد!', { replyTo: messageId });
        }
        return;
      ...
      if (command === '/unban') {
        ...
        if (user) {
          ...
          await this.helper.sendMessage(targetId, '✅ شما آنبن شدید!');
        } else {
          await this.helper.sendMessage(chatId, '❌ کاربر یافت نشد!', { replyTo: messageId });
        }
        return;
      ...
      if (command === '/setlimit') {
        if (args.length < 2) {
          await this.helper.sendMessage(chatId, '❌ استفاده: `/setlimit [نوع] [مقدار]`\nانواع: messages, voices, images, bots', { replyTo: messageId });
          return;
        }
        const type = args[0];
        const value = parseInt(args[1]);
        if (!['messages', 'voices', 'images', 'bots'].includes(type)) {
          await this.helper.sendMessage(chatId, '❌ نوع نامعتبر!', { replyTo: messageId });
          return;
        }
        this.limitSystem.FREE_LIMITS[type] = value;
        await this.helper.sendMessage(chatId, `✅ محدودیت ${type} به ${value} تغییر یافت!`, { replyTo: messageId });
        return;
      }

      if (command === '/reset') {
        if (args.length === 0) {
          await this.helper.sendMessage(chatId, '❌ استفاده: `/reset [آیدی کاربر]`', { replyTo: messageId });
          return;
        }
        const targetId = args[0];
        const key = `usage_${targetId}`;
        await this.db.delete(key);
        await this.helper.sendMessage(chatId, `✅ مصرف کاربر ${targetId} ریست شد!`, { replyTo: messageId });
        return;
      }
      ...
      if (command === '/logs') {
        ...
        if (logs.length === 0) {
          await this.helper.sendMessage(chatId, '📋 هیچ لاگی وجود ندارد!', { replyTo: messageId });
          return;
        }
        let text = '📋 **لاگ‌های اخیر**\n\n';
        for (const log of logs) {
          const emoji = log.level === 'error' ? '❌' : log.level === 'warning' ? '⚠️' : 'ℹ️';
          text += `${emoji} ${log.message}\n`;
          text += `🕐 ${new Date(log.timestamp).toLocaleString('fa-IR')}\n\n`;
        }
        await this.helper.sendMessage(chatId, text, { replyTo: messageId });
        return;
      }
  ...
  async handleCallback(callbackQuery) {
    try {
      ...
      // ===== ساخت ربات (مراحل) =====
      if (data === 'bb_start') {
        const result = await this.botBuilder.startBuild(userId);
        if (result.error) {
          await this.helper.answerCallback(callbackQuery.id, result.error, true);
          return;
        }
        await this.helper.editMessage(chatId, messageId, result.message, {
          keyboard: result.keyboard || undefined
        });
        return;
      }

      if (data.startsWith('build_pers_') || data === 'build_final' || data === 'build_cancel') {
        const result = await this.botBuilder.processBuildStep(userId, null, data);
        if (result.done) {
          await this.helper.editMessage(chatId, messageId, result.message, {
            keyboard: result.keyboard || this.keyboard.getKeyboard('main')
          });
      ...
      if (data === 'buy_coins') {
        await this.helper.editMessage(chatId, messageId, 
          ...
          '💎 ۵۰۰ سکه = ۴۰,۰۰۰ تومان\n' +
          '💎 ۱۰۰۰ سکه = ۷۰,۰۰۰ تومان\n\n' +
      ...
      if (data.startsWith('bb_stats_')) {
        const botId = data.replace('bb_stats_', '');
        const bot = await this.botBuilder.getBot(botId);
        if (!bot) {
          await this.helper.answerCallback(callbackQuery.id, '❌ ربات یافت نشد!', true);
          return;
        }
        if (bot.owner !== userId && !isAdmin) {
          await this.helper.answerCallback(callbackQuery.id, '❌ شما مالک این ربات نیستید!', true);
          return;
        }
        const stats = await this.botBuilder.getBotStats(botId);
        
        let text = `📊 **آمار ربات ${bot.name}**\n\n`;
        text += `👥 کاربران: ${stats.totalUsers}\n`;
        text += `🟢 آنلاین: ${stats.activeUsers}\n`;
        text += `💬 پیام‌ها: ${stats.messages}\n`;
        text += `🎨 تصاویر: ${stats.images}\n`;
        text += `🎤 ویس‌ها: ${stats.voices}\n`;
        text += `📅 ساخته شده: ${new Date(bot.createdAt).toLocaleDateString('fa-IR')}\n`;
        text += `⏱️ آپتایم: ${Math.floor(stats.uptime / (24*60*60*1000))} روز`;

        await this.helper.editMessage(chatId, messageId, text, {
          keyboard: this.botBuilder.getManageKeyboard(botId)
        });
        return;
      }

      if (data.startsWith('bb_delete_')) {
        const botId = data.replace('bb_delete_', '');
        const bot = await this.botBuilder.getBot(botId);
        if (!bot) {
          await this.helper.answerCallback(callbackQuery.id, '❌ ربات یافت نشد!', true);
          return;
      ...
      if (data === 'my_status') {
        ...
        if (await this.db.isPremium(userId)) {
        ...
        await this.helper.editMessage(chatId, messageId, response, {
          keyboard: this.keyboard.getKeyboard('main')
        });
        return;
      }
      ...
      await this.helper.answerCallback(callbackQuery.id, '❌ خطا رخ داد!', true);
    }
  }
}
  ...
  async fetch(request, env) {
    try {
      ...
      if (path.startsWith('/payment/')) {
        ...
        try {
          const result = await botHandler.botBuilder.confirmPayment(paymentId);
          if (result) {
            return new Response(`
              <html>
                <body style="font-family: Arial; text-align: center; padding: 50px; direction: rtl;">
                  ...
                  <a href="https://t.me/${botHandler.helper.env.BOT_USERNAME}">بازگشت به ربات</a>
                </body>
              </html>
